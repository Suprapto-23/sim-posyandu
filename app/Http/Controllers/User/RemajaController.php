...
class RemajaController extends Controller
{
    ...
    {
        if (! method_exists($remaja, 'kunjungans')) {
            return collect();
        }
        ...
        try {
            ...
            Log::warning('User remaja riwayat kunjungan skipped', [
                'remaja_id' => $remaja->id,
                'message' => $e->getMessage(),
            ]);
    ...
    private function getRiwayatFromPemeriksaan(Remaja $remaja): Collection
    {
        ...
        try {
            $query = Pemeriksaan::query()
                ...
                ->where(function (Builder $q) use ($remaja) {
                    ...
                    if ($hasKategoriPasien) {
                        $q->where(function (Builder $inner) use ($remaja) {
                            $inner->where('kategori_pasien', 'remaja')
                                ->where('pasien_id', $remaja->id);
                        });
                    }

                    if (Schema::hasColumn('pemeriksaans', 'remaja_id')) {
                        $hasKategoriPasien
                            ? $q->orWhere('remaja_id', $remaja->id)
                            : $q->where('remaja_id', $remaja->id);
                    }
                });
            ...
            Log::warning('User remaja riwayat pemeriksaan skipped', [
                'remaja_id' => $remaja->id,
                'message' => $e->getMessage(),
            ]);
    ...
    private function resolveLatestPemeriksaan(Remaja $remaja, Collection $riwayat): ?Pemeriksaan
    {
        ...
        try {
            ...
            Log::warning('User remaja latest pemeriksaan skipped', [
                'remaja_id' => $remaja->id,
                'message' => $e->getMessage(),
            ]);
    ...
    private function buildMetrics(
        ...
        $berat = $latestPemeriksaan->berat_badan
            ?? $remaja->berat_badan
            ?? null;

        $tinggi = $latestPemeriksaan->tinggi_badan
            ?? $remaja->tinggi_badan
            ?? null;
        ...
        return [
            [
                'label' => 'Usia',
                'value' => $this->ageText($remaja->tanggal_lahir ?? null),
                'caption' => 'Kategori remaja',
                'tone' => 'emerald',
            ],
            [
                'label' => 'IMT',
            ...
            [
                'label' => 'Berat',
                'value' => $this->numberValue($berat, 'kg'),
                'caption' => 'Pemeriksaan terakhir',
                'tone' => 'sky',
            ],
            [
                'label' => 'Tinggi',
                'value' => $this->heightValue($tinggi),
                'caption' => 'Pemeriksaan terakhir',
                'tone' => 'amber',
            ],
        ];
    ...
    private function buildTrend(Collection $riwayat): array
    {
        ...
        $values = $items
            ->map(fn ($item) => $this->resolveImt($item->pemeriksaan ?? null))
        ...
        return $items
            ->map(function ($item) use ($max) {
                ...
                return [
                    ...
                    'height' => filled($imt)
                        ? max(18, min(100, (int) round(($imt / $max) * 100)))
                        : 14,
                ];
            })
            ->toArray();
    ...
    private function buildImtAnalysis(?float $imt): array
    {
        if (! $imt) {
            return [
                ...
                'message' => 'Data berat badan dan tinggi badan belum tersedia.',
                'suggestion' => 'Lakukan pemeriksaan BB dan TB di Posyandu.',
                'tone' => 'slate',
                'icon' => 'fa-circle-info',
            ];
        }
        ...
        if ($imt < 18.5) {
            return [
                'label' => 'Kurus',
                'message' => 'IMT berada di bawah rentang normal.',
                'suggestion' => 'Perhatikan asupan protein, karbohidrat, dan konsultasikan dengan Bidan.',
                'tone' => 'amber',
        ...
        if ($imt < 25) {
            return [
                'label' => 'Normal',
                'message' => 'IMT berada pada rentang normal.',
        ...
        if ($imt < 30) {
            return [
                'label' => 'Berlebih',
                'message' => 'IMT mulai melebihi rentang normal.',
                'suggestion' => 'Kurangi makanan tinggi gula dan tingkatkan aktivitas fisik.',
                'tone' => 'rose',
        ...
        return [
            'label' => 'Obesitas',
            'message' => 'IMT berada jauh di atas rentang normal.',
