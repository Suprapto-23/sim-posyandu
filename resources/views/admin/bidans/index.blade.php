...
@php
    ...
    $totalData = isset($bidans) && method_exists($bidans, 'total')
        ? $bidans->total()
        : count($bidans ?? []);

    $rangeText = isset($bidans) && method_exists($bidans, 'firstItem')
        ? 'Menampilkan ' . (($bidans->firstItem() ?? 0)) . ' sampai ' . (($bidans->lastItem() ?? 0)) . ' dari ' . $bidans->total() . ' akun bidan'
        : 'Menampilkan ' . count($bidans ?? []) . ' akun bidan';

    $getName = function ($bidan) {
        return $bidan->profile?->full_name ?? $bidan->name ?? 'Bidan';
    };

    $getNik = function ($bidan) {
        return $bidan->nik ?? $bidan->profile?->nik ?? '-';
    };

    $getPhone = function ($bidan) {
        return $bidan->profile?->telepon ?? '-';
    };

    $getJabatan = function ($bidan) {
        return $bidan->bidan?->jabatan ?? 'Bidan';
    };
...
<style>
    ...
    .admin-bidan-modal-backdrop.is-open {
        display: flex !important;
    }

    .admin-bidan-modal-card {
        width: min(100%, 500px);
    ...
    .admin-bidan-modal-backdrop.is-open .admin-bidan-modal-card {
...
@section('content')
<div class="admin-bidan-page relative min-h-screen overflow-hidden px-4 py-5 sm:px-6 lg:px-8">
    <div class="pointer-events-none absolute inset-0 admin-bidan-grid opacity-70"></div>

    <div class="relative z-10 mx-auto max-w-[1280px] space-y-5">

        <section class="admin-bidan-glass overflow-hidden rounded-[1.65rem] border border-white/80 p-5">
            <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                <div class="min-w-0">
                    ...
                    <h1 class="mt-3 text-2xl font-black tracking-tight text-slate-950">
                        Manajemen Akun Bidan
                    </h1>

                    <p class="mt-2 max-w-3xl text-sm font-semibold leading-6 text-slate-600">
                        Kelola akun Bidan untuk validasi pemeriksaan, jadwal pelayanan, imunisasi, dan rekam medis Posyandu.
                    </p>
                </div>
        ...
        <section class="grid gap-4 md:grid-cols-3">
            <div class="admin-bidan-glass rounded-[1.4rem] border border-emerald-200 p-4">
            ...
            <div class="admin-bidan-glass rounded-[1.4rem] border border-sky-200 p-4">
                <div class="flex items-center justify-between gap-4">
                    <div>
            ...
            <div class="admin-bidan-glass rounded-[1.4rem] border border-rose-200 p-4">
                <div class="flex items-center justify-between gap-4">
                    <div>
        ...
        <section class="admin-bidan-glass rounded-[1.5rem] border border-white/80 p-4">
            <form method="GET"
                  action="{{ route('admin.bidans.index') }}"
                  class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_180px_140px_auto_auto] lg:items-end">
                <div>
                    <label for="search" class="admin-bidan-label">Cari Bidan</label>
                    <div class="relative">
                        ...
                        <input id="search"
                               ...
                               value="{{ $search }}"
                               autocomplete="off"
                               class="admin-bidan-field pl-11"
                               placeholder="Cari nama, email, NIK, atau telepon">
                    </div>
                </div>

                <div>
                    <label for="status" class="admin-bidan-label">Status</label>
                    <select id="status" name="status" class="admin-bidan-field">
                ...
                <div>
                    <label for="per_page" class="admin-bidan-label">Tampil</label>
                    <select id="per_page" name="per_page" class="admin-bidan-field">
        ...
        <section class="admin-bidan-glass rounded-[1.5rem] border border-white/80 p-4">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($bidans ?? [] as $bidan)
                            @php
                                $name = $getName($bidan);
                                $nik = $getNik($bidan);
                                $phone = $getPhone($bidan);
                                $jabatan = $getJabatan($bidan);
                            @endphp
                            ...
                            <tr class="transition hover:bg-emerald-50/35">
                                ...
                                <td class="px-4 py-4">
                                    <span class="admin-bidan-chip {{ $statusBadge($bidan->status) }}">
                                        <span class="h-2 w-2 rounded-full {{ $statusDot($bidan->status) }}"></span>
                                        {{ $statusText($bidan->status) }}
                                    </span>
                                </td>
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex justify-end gap-2">
                                        <a href="{{ route('admin.bidans.show', $bidan->id) }}"
                                           class="admin-bidan-action border border-slate-200 bg-white text-slate-700 hover:bg-slate-50"
                                           title="Detail">
                                            <i class="fas fa-eye"></i>
                                        </a>

                                        <a href="{{ route('admin.bidans.edit', $bidan->id) }}"
                                           class="admin-bidan-action border border-sky-200 bg-sky-50 text-sky-700 hover:bg-sky-100"
                                           title="Edit">
                                        ...
                                        <form action="{{ route('admin.bidans.reset-password', $bidan->id) }}"
                                            ...
                                            <button type="submit"
                                                    class="admin-bidan-action border border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100"
                                        ...
                                        <form action="{{ route('admin.bidans.destroy', $bidan->id) }}"
                                              ...
                                              data-action-title="Hapus akun bidan?"
                                            ...
                                            <button type="submit"
                                                    class="admin-bidan-action border border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100"
                                                    title="Hapus">
                            ...
                            <tr>
                                <td colspan="5" class="px-4 py-12 text-center">
                                    ...
                                    <p class="mt-2 text-sm font-semibold text-slate-500">
                                        Tambahkan akun Bidan agar fitur medis dapat digunakan.
                                    </p>
                                </td>
                            </tr>
                        @endforelse
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <div class="mt-3 grid gap-2 sm:grid-cols-2">
                                    <div class="admin-bidan-soft rounded-2xl p-3">
                                ...
                                <div class="mt-4 grid grid-cols-2 gap-2">
                                    ...
                                    <form action="{{ route('admin.bidans.reset-password', $bidan->id) }}"
                                        ...
                                        <button type="submit"
                                                class="admin-bidan-action w-full border border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100">
                                            <i class="fas fa-key"></i>
                                            Password
                                        </button>
                                    </form>
                                    ...
                                    <form action="{{ route('admin.bidans.destroy', $bidan->id) }}"
                                        ...
                                        <button type="submit"
                                                class="admin-bidan-action w-full border border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100">
                                            <i class="fas fa-trash-alt"></i>
                                            Hapus
                                        </button>
                                    </form>
            ...
            @if(isset($bidans) && method_exists($bidans, 'hasPages') && $bidans->hasPages())
    ...
    <div id="adminBidanActionModal" class="admin-bidan-modal-backdrop" aria-hidden="true">
        <div class="admin-bidan-modal-card p-6">
            <div class="flex gap-4">
                <div id="adminBidanActionIconBox" class="flex h-12 w-12 shrink-0 items-center justify-center rounded-[1.25rem] bg-gradient-to-br from-emerald-500 to-teal-500 text-white shadow-lg shadow-emerald-500/20">
                    <i id="adminBidanActionIcon" class="fas fa-check-circle text-xl"></i>
                </div>

                <div class="min-w-0 flex-1">
                    <p id="adminBidanActionEyebrow" class="text-[11px] font-black uppercase tracking-[.18em] text-emerald-700">
                        Konfirmasi
                    </p>

                    <h3 id="adminBidanActionTitle" class="mt-1 text-lg font-black text-slate-950">
                        Konfirmasi aksi?
                    </h3>

                    <p id="adminBidanActionMessage" class="mt-2 text-sm font-semibold leading-6 text-slate-600">
...
(function () {
    ...
    const modal = document.querySelector('#adminBidanActionModal');
    const modalTitle = document.querySelector('#adminBidanActionTitle');
    const modalMessage = document.querySelector('#adminBidanActionMessage');
    const modalEyebrow = document.querySelector('#adminBidanActionEyebrow');
    const modalIconBox = document.querySelector('#adminBidanActionIconBox');
    const modalIcon = document.querySelector('#adminBidanActionIcon');
    const modalCancel = document.querySelector('#adminBidanActionCancel');
    const modalSubmit = document.querySelector('#adminBidanActionSubmit');
