...
<div id="adminMain" class="max-w-6xl mx-auto space-y-8 pb-12">
    ...
    <section class="bg-gradient-to-br from-blue-600 via-sky-500 to-emerald-400 rounded-[2.5rem] p-9 md:p-10 relative overflow-hidden shadow-[0_20px_40px_-10px_rgba(59,130,246,.30)] border border-white/20 flex flex-col items-center justify-center text-center group animate-pop-in">
        ...
        <div class="relative z-10">
            ...
            <a href="{{ route('admin.bidans.create') }}" class="inline-flex items-center gap-2 bg-white hover:bg-slate-50 text-blue-600 font-black px-7 py-3.5 rounded-xl transition-all shadow-lg hover:shadow-xl hover:-translate-y-1">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xs font-medium text-blue-600">
                        Password baru berhasil dibuat untuk
                        <strong class="text-blue-800">{{ session('user_name') ?? session('reset_name') ?? 'Bidan' }}</strong>.
                    </p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden animate-pop-in delay-200 relative z-10 mt-4">
        ...
        <div class="px-8 py-6 border-b border-slate-50 flex flex-col sm:flex-row justify-between items-center gap-4">
            <div class="flex flex-col">
                ...
                <p class="text-[11px] font-bold text-slate-400 mt-1 uppercase tracking-widest ml-[52px]">
                    Total tampil:
                    <span id="totalDataCount" class="text-blue-500">{{ $totalData }}</span> Data
                </p>
            </div>
            ...
            <table class="w-full text-left border-collapse min-w-[900px]">
                ...
                <tbody id="tableBody" class="text-sm font-medium text-slate-600" style="transition: opacity 0.3s ease-in-out;">
                    @forelse($bidans ?? [] as $bidan)
                        @php
                            $name = $getName($bidan);
                            $userStatus = $bidan->status ?? 'inactive';
                        @endphp
                        ...
                        <tr class="table-row-soft border-b border-slate-50 text-center">
                            ...
                            <td class="py-4 px-6 align-middle">
                                <div class="flex items-center justify-center gap-2 whitespace-nowrap w-max mx-auto">
                                    <a href="{{ route('admin.bidans.show', $bidan->id) }}" class="action-soft w-8 h-8 rounded-lg bg-sky-50 text-sky-600 hover:bg-sky-500 hover:text-white flex items-center justify-center transition-all" title="Lihat Detail">
                                        <i class="fas fa-eye"></i>
                                    </a>

                                    <a href="{{ route('admin.bidans.edit', $bidan->id) }}" class="action-soft w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white flex items-center justify-center transition-all" title="Edit Data">
                                        <i class="fas fa-edit"></i>
                                    </a>

                                    <form action="{{ route('admin.bidans.toggle-status', $bidan->id) }}" method="POST" class="inline dynamic-form-toggle">
                                        @csrf
                                        @method('PATCH')
                                        @if($userStatus === 'active')
                                    ...
                                    <form action="{{ route('admin.bidans.reset-password', $bidan->id) }}" method="POST" class="inline dynamic-form-reset">
                                        @csrf
                                    ...
                                    <form action="{{ route('admin.bidans.destroy', $bidan->id) }}" method="POST" class="inline dynamic-form-delete">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="6" class="py-16 text-center">
                                <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-slate-50 text-slate-300 mb-4 border border-slate-100">
                                    <i class="fas fa-user-md text-3xl opacity-50"></i>
                                </div>
                                <h4 class="text-sm font-black text-slate-400 uppercase tracking-widest mb-1">
                                    Pencarian Kosong
                                </h4>
                                <p class="text-xs font-medium text-slate-400">
                                    Sistem tidak dapat menemukan data bidan pada pencarian ini.
                                </p>
                            </td>
                        </tr>
                    @endforelse
        ...
        @if(isset($bidans) && method_exists($bidans, 'hasPages') && $bidans->hasPages())
            @php $bidans->withQueryString(); @endphp
            <div id="tableFooter" class="border-t border-slate-100 bg-slate-50/50 px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-4">
                <p class="text-[10px] font-black uppercase tracking-widest text-slate-500">
                    Halaman <span class="text-slate-900">{{ $bidans->currentPage() }}</span> dari <span class="text-slate-900">{{ $bidans->lastPage() }}</span>
                </p>
                ...
                <div class="flex items-center gap-2">
                    ...
                    @else
                        <a href="{{ $bidans->previousPageUrl() }}" class="btn-pill w-10 h-10 flex items-center justify-center border border-slate-200 bg-white text-slate-600 shadow-sm hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-200 transition-all">
                            <i class="fas fa-chevron-left text-xs"></i>
                        </a>
                    @endif
                    ...
                    {{-- Page Numbers --}}
                    @php
                        $start = max(1, $bidans->currentPage() - 2);
                        $end = min($bidans->lastPage(), $bidans->currentPage() + 2);
                    @endphp
                    @for ($page = $start; $page <= $end; $page++)
                        ...
                        @else
                            <a href="{{ $bidans->url($page) }}" class="btn-pill w-10 h-10 flex items-center justify-center border border-slate-200 bg-white text-slate-600 font-bold text-sm shadow-sm hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-200 transition-all">
                                {{ $page }}
                            </a>
                        @endif
                    @endfor
                    ...
                    @if ($bidans->hasMorePages())
                        <a href="{{ $bidans->nextPageUrl() }}" class="btn-pill w-10 h-10 flex items-center justify-center border border-slate-200 bg-white text-slate-600 shadow-sm hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-200 transition-all">
...
(function() {
    ...
    container.addEventListener('click', function(e) {
        ...
        if (toggleBtn) {
            ...
            const name = toggleBtn.dataset.name || 'bidan';
        ...
        if (resetBtn) {
            ...
            const name = resetBtn.dataset.name || 'bidan';
        ...
        if (deleteBtn) {
            ...
            const name = deleteBtn.dataset.name || 'bidan';
    ...
    // --- 3. FUNGSI FETCH DATA AJAX (Hanya Search Parameter) ---
