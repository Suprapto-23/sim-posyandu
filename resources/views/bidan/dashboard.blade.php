...
@push('styles')
<style>
    /* OPTIMASI SCROLLING 1: Smooth Behavior di root */
    html { scroll-behavior: smooth; }

    /* OPTIMASI SCROLLING 2: Background dipisah agar tidak perlu re-paint saat di-scroll */
    ...
    /* OPTIMASI SCROLLING 3: Membuang backdrop-filter pada card biasa, gunakan GPU Acceleration */
    .widget-card {
        background: #ffffff; /* Solid White sangat ringan untuk browser */
    ...
    /* OPTIMASI SCROLLING 4: Touch Scrolling yang fluid di perangkat mobile */
    .slim-scroll {
...
@section('content')
{{-- Layer Background Statis (Bikin Scroll Mulus) --}}
<div class="bg-mesh-fixed"></div>

<div class="px-4 py-8 sm:px-6 lg:px-8 max-w-[1400px] mx-auto space-y-8">

    {{-- 1. FLOATING HERO WIDGET (Tetap dipertahankan kemewahannya) --}}
    <div class="relative overflow-hidden rounded-[3rem] bg-gradient-to-r from-teal-500 via-teal-400 to-emerald-400 p-8 sm:p-10 shadow-2xl shadow-teal-500/20 flex flex-col lg:flex-row justify-between items-center gap-8 border-[6px] border-white/40" style="transform: translateZ(0);">
        ...
        <div class="relative z-10 w-full lg:w-1/2 flex flex-col gap-5 text-center lg:text-left">
            ...
            <div class="flex flex-wrap justify-center lg:justify-start gap-4 mt-2">
                @if(Route::has('bidan.pemeriksaan.index'))
                    <a href="{{ route('bidan.pemeriksaan.index', ['tab' => 'pending']) }}" class="btn-pill bg-white text-teal-600 hover:text-teal-800 hover:bg-teal-50 px-6 py-3 text-sm font-bold shadow-lg flex items-center gap-2">
                        <i class="fa-solid fa-clipboard-check"></i> Validasi Baru
                        @if($pendingCount > 0)
                            <span class="bg-rose-500 text-white text-[10px] font-black px-2 py-0.5 rounded-full ml-1 animate-pulse">{{ $pendingCount }}</span>
                        @endif
                    </a>
                @endif
                @if(Route::has('bidan.jadwal.create'))
                    <a href="{{ route('bidan.jadwal.create') }}" class="btn-pill bg-black/10 hover:bg-black/20 text-white border border-white/30 px-6 py-3 text-sm font-bold backdrop-blur-md transition-all flex items-center gap-2">
        ...
        <div class="relative z-10 w-full lg:w-auto flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <div class="widget-card !rounded-[2.5rem] p-6 flex flex-col items-center justify-center min-w-[200px]">
            ...
            <div class="widget-card !rounded-[2.5rem] p-6 flex flex-col justify-center gap-4 min-w-[200px]">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-full bg-amber-100 text-amber-500 flex items-center justify-center text-xl">
                        <i class="fa-solid fa-file-medical"></i>
                    </div>
                    <div>
                        <p class="text-[10px] font-bold text-slate-400 uppercase">Tervalidasi Bulan Ini</p>
                        <p class="text-2xl font-black text-slate-800">{{ number_format(data_get($stats, 'tervalidasi', 0)) }}</p>
                    </div>
                </div>
                <div class="w-full border-t-[3px] border-dotted border-slate-100 rounded-full"></div>
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-full bg-blue-100 text-blue-500 flex items-center justify-center text-xl">
                        <i class="fa-solid fa-syringe"></i>
                    </div>
                    <div>
                        <p class="text-[10px] font-bold text-slate-400 uppercase">Imunisasi Bulan Ini</p>
                        <p class="text-2xl font-black text-slate-800">{{ number_format(data_get($stats, 'imunisasi_bulan_ini', 0)) }}</p>
                    </div>
                </div>
            </div>
        ...
        <div class="xl:col-span-2 flex flex-col gap-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                @foreach($breakdowns as $b)
                    <div class="widget-card !rounded-full p-3 pr-6 flex items-center gap-4 cursor-default">
                        <div class="w-12 h-12 rounded-full bg-{{ $b['color'] }}-50 text-{{ $b['color'] }}-600 flex items-center justify-center text-xl shrink-0">
            ...
            <div class="widget-card p-6 sm:p-8 flex-1 flex flex-col min-h-[350px]">
                <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 bg-slate-50/50 p-2 pl-6 rounded-full border border-slate-100">
                    <div>
                        <h2 class="text-sm font-bold text-slate-800 uppercase tracking-widest">Tren Layanan Pemeriksaan</h2>
                    </div>
                    <div class="flex shrink-0 bg-white p-1 rounded-full shadow-sm border border-slate-100 relative">
                        <button type="button" class="btn-pill px-4 py-2 text-xs font-bold transition-all duration-300 bg-teal-500 text-white shadow-md">
                            6 Bulan Terakhir
                        </button>
                    </div>
                </div>
                <div id="bidanTrendChart" class="w-full flex-1" style="min-height: 250px;"></div>
            </div>
        </div>

        {{-- Kanan: Antrean Pemeriksaan Masuk --}}
        <div class="widget-card flex flex-col p-2 h-[550px] xl:h-auto">
            <div class="p-5 flex items-center justify-between pb-2 border-b border-slate-50">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full bg-amber-50 text-amber-500 flex items-center justify-center text-lg shadow-inner border border-amber-100">
                        <i class="fa-solid fa-list-check"></i>
                    </div>
                    <h2 class="text-sm font-bold text-slate-800 uppercase tracking-widest">Antrean Validasi</h2>
                </div>
                @if(Route::has('bidan.pemeriksaan.index'))
                    <a href="{{ route('bidan.pemeriksaan.index') }}" class="btn-pill w-10 h-10 bg-slate-50 hover:bg-teal-50 border border-slate-100 text-slate-400 hover:text-teal-600 flex items-center justify-center transition-all">
                        <i class="fa-solid fa-arrow-right"></i>
                    </a>
                @endif
            </div>

            <div class="flex-1 overflow-y-auto slim-scroll p-3 space-y-3">
                @forelse($recentPemeriksaans as $item)
                    ...
                    <a href="{{ data_get($item, 'id') ? $routeUrl('bidan.pemeriksaan.validasi', data_get($item, 'id')) : '#' }}" 
                       class="group block rounded-[2rem] p-3 border transition-all {{ $isPending ? 'bg-gradient-to-r from-amber-50 to-orange-50 border-amber-200 shadow-sm' : 'bg-white border-slate-100 hover:border-teal-200 hover:shadow-md' }}">
                        <div class="flex gap-4 items-center">
                            <div class="flex flex-col items-center justify-center w-14 h-14 rounded-full shrink-0 shadow-inner {{ $isPending ? 'bg-amber-500 text-white shadow-amber-500/30' : 'bg-slate-50 text-slate-600 group-hover:bg-teal-500 group-hover:text-white' }} transition-colors">
                                <i class="fa-solid fa-file-medical text-xl"></i>
                            </div>
                            <div class="min-w-0 flex-1 pr-2">
                                <div class="flex items-center gap-2 mb-1">
                                    <h3 class="truncate text-sm font-bold text-slate-800">{{ data_get($item, 'nama', '-') }}</h3>
                                    @if($isPending)
                                        <span class="btn-pill bg-amber-500 px-2 py-0.5 text-[9px] font-black text-white uppercase tracking-wider">TINJAU</span>
                                    @endif
                                </div>
                                <p class="btn-pill inline-flex bg-white border border-slate-100 px-2.5 py-1 text-[10px] font-bold text-slate-500 items-center gap-1.5 shadow-sm">
                ...
                @empty
                    <div class="flex h-full flex-col items-center justify-center text-center opacity-70 p-8 min-h-[250px]">
                        ...
                        <p class="text-sm font-bold text-slate-500">Semua Tervalidasi</p>
                        <p class="text-xs font-semibold text-slate-400 mt-2">Tidak ada pemeriksaan baru dari Kader yang perlu ditinjau hari ini.</p>
                    </div>
                @endforelse
            </div>
        </div>
