...
@section('title', ($mode ?? 'create') === 'edit' ? 'Edit Jadwal Posyandu' : 'Buat Jadwal Posyandu')
@section('page-name', 'Kelola Jadwal')
@section('page-title', ($mode ?? 'create') === 'edit' ? 'Edit Jadwal Posyandu' : 'Buat Jadwal Posyandu')

@php
    use Carbon\Carbon;

    $mode = $mode ?? 'create';
    $isEdit = $mode === 'edit' && !empty($jadwal);
    ...
    $pageTitle = $isEdit ? 'Edit Jadwal Posyandu' : 'Buat Jadwal Baru';
    $pageDesc = $isEdit ? 'Perbaiki rincian agenda pelayanan selama jadwal masih berstatus aktif.' : 'Susun agenda pelayanan baru dan tentukan target sasaran Posyandu.';
    $formAction = $isEdit ? route('bidan.jadwal.update', $jadwal->id) : route('bidan.jadwal.store');
    $submitLabel = $isEdit ? 'Simpan Perubahan' : 'Simpan Jadwal Baru';
@endphp
...
{{-- Wrapper difokuskan ke tengah (max-w-[1000px]) --}}
    ...
    <section class="relative overflow-hidden rounded-[3rem] bg-gradient-to-r from-emerald-500 via-teal-500 to-green-500 p-8 sm:p-10 shadow-2xl shadow-emerald-500/20 flex flex-col md:flex-row justify-between items-center gap-8 border-[6px] border-white/40">
        ...
        <div class="relative z-10 w-full flex flex-col gap-4 text-center md:text-left">
            <div class="flex flex-col sm:flex-row items-center gap-3">
                <a href="{{ $isEdit ? route('bidan.jadwal.show', $jadwal->id) : route('bidan.jadwal.index') }}" class="btn-pill inline-flex items-center gap-2 border border-white/30 bg-white/20 px-5 py-2 text-[10px] font-black uppercase tracking-widest text-white hover:bg-white/30 shadow-sm backdrop-blur-md transition-colors">
                ...
                <span class="inline-flex items-center gap-1.5 rounded-full border border-white/30 bg-white/20 px-3 py-1.5 text-[9px] font-black uppercase tracking-widest text-white shadow-sm backdrop-blur-md">
                    <i class="fa-solid fa-calendar-check"></i> {{ $isEdit ? 'Mode Edit' : 'Agenda Baru' }}
                </span>
            </div>
    ...
    <form id="jadwalForm" method="POST" action="{{ $formAction }}">
        ...
        <section class="widget-card overflow-hidden flex flex-col w-full border-t-4 border-t-teal-500">
            ...
            <div class="bg-slate-50/70 px-6 py-6 border-b border-slate-100 flex items-center gap-4 shrink-0">
                <div class="w-12 h-12 rounded-2xl bg-white border border-slate-100 text-teal-500 flex items-center justify-center text-xl shadow-sm"><i class="fa-solid fa-clipboard-list"></i></div>
                <div>
                    <h2 class="text-base font-black text-slate-800 uppercase tracking-tight">Formulir Setup Jadwal</h2>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Lengkapi parameter di bawah</p>
                </div>
            </div>
            ...
            <div class="p-6 md:p-8 space-y-8">
                ...
                <div class="space-y-6 bg-slate-50/50 p-5 md:p-6 rounded-3xl border border-slate-100">
                    <div>
                        <label class="form-label mb-3">1. Pilih Kategori Layanan <span class="req-star">*</span></label>
                ...
                <div class="bg-amber-50/50 p-5 md:p-6 rounded-3xl border border-amber-100">
                    <label class="form-label mb-3 text-amber-700">Status Pelaksanaan (Hanya untuk Edit) <span class="req-star">*</span></label>
            ...
            <div class="p-6 md:p-8 bg-slate-50/50 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-end gap-3 mt-auto shrink-0">
                <a href="{{ $isEdit ? route('bidan.jadwal.show', $jadwal->id) : route('bidan.jadwal.index') }}" class="w-full sm:w-auto px-8 py-3.5 rounded-xl font-bold text-slate-500 bg-white border border-slate-200 hover:bg-slate-50 hover:text-slate-700 transition-all shadow-sm text-sm text-center">
                    Batal
                </a>
