...
<style>
    :root {
        ...
        --a500:#f59e0b;--s900:#0f172a;--s800:#1e293b;--s700:#334155;--s600:#475569;
        --s500:#64748b;--s400:#94a3b8;--s200:#e2e8f0;--s100:#f1f5f9;
        --ease:cubic-bezier(.16,1,.3,1);
    }

    .login-shell {
        width:100%;max-width:1280px;min-height:min(700px,calc(100svh - 36px));
        margin:0 auto;padding:0 32px;
        display:grid;grid-template-columns:minmax(0,1fr) minmax(420px,.88fr);
        align-items:center;gap:56px;position:relative;
    }
    ...
    .brand-logo {
        width:390px;max-width:100%;height:auto;margin-bottom:16px;
        filter:drop-shadow(0 14px 24px rgba(5,150,105,.08));
        user-select:none;pointer-events:none;
    }
    .brand-title { margin:0;color:var(--s900);font-size:25px;font-weight:900;line-height:1.28;letter-spacing:-.05em; }
    .brand-divider { display:flex;align-items:center;justify-content:center;gap:13px;margin:14px 0 17px; }
    ...
    .bdd {
        width:8px;height:8px;border-radius:2px;background:var(--a500);
        transform:rotate(45deg);box-shadow:0 4px 10px rgba(245,158,11,.26);
    }
    .brand-desc { max-width:420px;margin:0 0 24px;color:var(--s500);font-size:15px;line-height:1.7;font-weight:650; }
    .feature-grid {
    ...
    .feature-box:hover { transform:translateY(-5px);background:rgba(255,255,255,.92);box-shadow:0 20px 36px rgba(16,185,129,.12); }
    .feature-icon {
        width:40px;height:40px;border-radius:15px;
        background:rgba(5,150,105,.10);color:var(--g600);
        display:flex;align-items:center;justify-content:center;font-size:17px;
    }
    .feature-text { color:var(--s800);font-size:11.5px;font-weight:900; }

    /* ── Card ── */
    .form-side { width:100%;display:flex;align-items:center;justify-content:center; }
    .login-card {
        position:relative;width:100%;max-width:500px;min-height:570px;
    ...
    .login-title  { margin:0 0 10px;color:var(--g900);font-size:29px;font-weight:900;line-height:1.14;letter-spacing:-.052em; }
    ...
    .field-label { display:block;margin:0 0 9px 2px;color:var(--s800);font-size:12px;font-weight:900;letter-spacing:.055em; }
    .field-wrap  { position:relative;width:100%; }
    .field-input {
        width:100%;height:56px;padding:0 52px 0 50px;
    ...
    .field-icon {
        ...
        color:var(--s400);font-size:17px;pointer-events:none;transition:color .18s ease;
    }
    ...
    .forgot-row  { display:flex;justify-content:flex-end;margin-top:-3px; }
    .forgot-link { color:var(--g600);font-size:13px;font-weight:900;text-decoration:none; }
    .forgot-link:hover { color:var(--g700);text-decoration:underline;text-underline-offset:4px; }

    .submit-wrap { padding-top:12px; }
    .submit-btn {
    ...
    .submit-btn:hover { transform:translateY(-2px);filter:saturate(1.05);box-shadow:0 21px 38px rgba(5,150,105,.32),inset 0 1px 0 rgba(255,255,255,.26); }
    .submit-btn:disabled { opacity:.88;cursor:wait;pointer-events:none;transform:none; }

    /* ── Info note ── */
    .info-note {
        margin-top:27px;padding:16px 17px;border-radius:20px;
    ...
    /* ── Animations ── */
    .anim-l { opacity:0;transform:translateX(-24px);animation:aL .46s var(--ease) forwards; }
    .anim-r { opacity:0;transform:translateX(24px); animation:aR .46s var(--ease) forwards; }
    .anim-u { opacity:0;transform:translateY(16px); animation:aU .38s var(--ease) forwards; }
    .d1{animation-delay:.04s}.d2{animation-delay:.08s}.d3{animation-delay:.12s}
    .d4{animation-delay:.16s}.d5{animation-delay:.20s}
    @keyframes aL { to{opacity:1;transform:translateX(0)} }
    @keyframes aR { to{opacity:1;transform:translateX(0)} }
    ...
    @media(max-width:1180px){
        ...
        .brand-logo{width:355px}.brand-title{font-size:23px}
        .login-card{max-width:480px;padding:39px 42px 36px}
    }
    @media(max-width:1024px){
        .login-shell{min-height:calc(100svh - 24px);display:flex;align-items:center;justify-content:center;padding:14px 0}
        .brand-side{display:none}
        .form-side{position:relative}
        .form-side::before{content:"";position:absolute;width:280px;height:280px;left:50%;top:50%;transform:translate(-50%,-50%);border-radius:999px;background:radial-gradient(circle,rgba(255,255,255,.62),rgba(255,255,255,.16) 54%,transparent 72%);z-index:0}
        .login-card{max-width:440px;min-height:auto;padding:27px 25px 25px;border-radius:31px;z-index:2;backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px)}
        .mobile-brand{display:flex}
        .login-header{margin-bottom:25px}
        .login-title{font-size:23px}.login-subtitle{font-size:13px}
        ...
        .info-note{margin-top:22px;padding:14px;border-radius:18px}
        .info-icon{width:34px;height:34px;border-radius:13px;font-size:14px}
        .info-title{font-size:12.8px}.info-text{font-size:11.8px}
    }
    @media(max-width:640px){
        .login-shell{min-height:calc(100svh - 18px);padding:8px 0}
        .login-card{max-width:100%;width:min(100%,388px);border-radius:28px;padding:24px 21px 22px}
        .mobile-brand img{width:145px}.login-title{font-size:21px}
        .login-subtitle{font-size:12.4px}.login-form{gap:16px}
        .forgot-link{font-size:12.2px}
    }
    @media(max-width:390px){
        .login-card{width:100%;border-radius:25px;padding:22px 17px 20px}
        .mobile-brand img{width:136px}.login-title{font-size:20px}
        .field-input{height:49px;font-size:12.6px}.info-text{font-size:11.5px}
    }
    @media(max-width:768px){
        .feature-box,.login-card,.info-note{backdrop-filter:none;-webkit-backdrop-filter:none}
        .anim-l,.anim-r,.anim-u{animation-duration:.28s}
    }
    @media(prefers-reduced-motion:reduce){
        .anim-l,.anim-r,.anim-u{opacity:1!important;transform:none!important;animation:none!important}
    }
</style>
@endpush
...
<div class="login-shell">
    ...
    <section class="brand-side anim-l" aria-hidden="true">
        <div class="brand-content">
            ...
            <div class="feature-grid">
                ...
                <div class="feature-box anim-u d{{ $i+1 }}">
    ...
    {{-- Login card --}}
    <section class="form-side anim-r d1">
        <div class="login-card">
            <div class="card-inner">
                ...
                <form id="loginForm" method="POST" action="{{ route('login.post') }}"
                      class="login-form" autocomplete="on" novalidate>
                    @csrf

                    <div class="field-group">
                        <label for="login_field" class="field-label">Email atau Username</label>
                        <div class="field-wrap">
                            <input type="text" id="login_field" name="login"
                                   value="{{ old('login') }}" class="field-input"
...
@push('scripts')
<script>
/* ── Nexus Alert — standalone untuk halaman auth ─────────────────
   Swal sudah di-load via layouts.auth (cdn sweetalert2).
   Kita definisikan helper lokal agar tidak bergantung
   pada nexusAlert dari layout kader/bidan.
─────────────────────────────────────────────────────────────────── */
(function () {
    function waitSwal(cb) {
        if (window.Swal) { cb(); return; }
        const t = setInterval(() => { if (window.Swal) { clearInterval(t); cb(); } }, 40);
    }

    window.authAlert = function (title, text, icon) {
        waitSwal(() => Swal.fire({
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    /* Jika ada error server → pastikan flag login bersih */
    @if($errors->any())
    ...
    form?.addEventListener('submit', function (e) {
        ...
        /* Tandai: halaman berikutnya (dashboard) tampilkan loading screen masuk */
        ...
        /* Trigger auth transition jika ada (layouts.auth) */
        window.PosyanduAuthTransition?.show?.('Membuka Portal');

        form.submit(); /* langsung, tanpa timeout */
    });

    /* ── Flash alerts ── */
    @if($errors->any())
        authAlert('Otentikasi Gagal', @json($errors->first()), 'error');
    @endif
    @if(session('info'))
        authAlert('Informasi Sistem', @json(session('info')), 'info');
    @endif
    @if(session('success'))
        authAlert('Berhasil', @json(session('success')), 'success');
    @endif
});
</script>
@endpush