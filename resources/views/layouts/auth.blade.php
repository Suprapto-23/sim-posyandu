...
<head>
    ...
    <!-- OPTIMASI LCP: preload logo dengan prioritas tertinggi -->
    ...
    <!--
        OPTIMASI FONT:
        - Hanya memuat 4 ketebalan yang benar-benar dipakai (400/500/700/800).
          Sebelumnya 600 & 900 ikut diunduh padahal tidak dipakai di CSS manapun.
        - fetchpriority="low" agar font tidak berebut bandwidth dengan logo (elemen LCP).
    -->
    ...
    <!--
        Font Awesome (CDN, ~37 KiB CSS tak terpakai) dan SweetAlert2 (~third-party JS) sudah dihapus.
        Semua ikon kini SVG inline (lihat sprite di bawah <body>) dan alert dibangun sendiri
        (lihat #premiumAlert), sehingga dua origin pihak ketiga ini tidak lagi memblokir render
        maupun menyumbang unused CSS/JS.
    -->
    ...
    <style>
        :root {
            ...
            --amber-500: #f59e0b;
            /*
                [BUG FIX] --slate-600 dan --amber-500 sebelumnya TIDAK didefinisikan di root,
                padahal dipakai di login.blade.php (placeholder, field-icon, titik amber di divider).
                Karena var() yang tidak terdefinisi membuat seluruh deklarasi properti itu invalid,
                fix kontras yang ditulis di komentar "[A11Y FIX]" sebelumnya diam-diam TIDAK pernah
                aktif. Ini kemungkinan besar penyebab skor Aksesibilitas mentok di 94, bukan 100.
            */
        }
        ...
        * { box-sizing: border-box; }
        ...
        /* Sprite ikon SVG: disembunyikan secara visual, tetap dibaca browser (menggantikan Font Awesome) */
        ...
        /* 1. EFEK BLUR BACKGROUND
           Dipecah jadi 2 layer: base (tajam) + blur (sudah di-blur secara statis).
           Yang berpindah hanya opacity, bukan filter, supaya animasinya composited
           (girak di GPU) dan tidak lagi kena flag "avoid non-composited animations". */
        .auth-bg { position: absolute; inset: 0; width: 100%; height: 100%; z-index: 1; overflow: hidden; }
        .auth-bg-base, .auth-bg-blur {
        ...
        .auth-bg-blur { filter: blur(12px); opacity: 0; }
        body.is-splitting .auth-bg-base { opacity: 0.1; }
        body.is-splitting .auth-bg-blur { opacity: 1; }

        /* 2. FORM TRANSISI KELUAR */
        .auth-main {
        ...
        @keyframes fluentSpin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }

        .bridge-loader p { color: var(--slate-900); font-weight: 800; font-size: 15px; margin: 0; letter-spacing: -0.3px; }

        /* 5. PREMIUM ALERT (pengganti SweetAlert2 — dibangun sendiri, tanpa dependensi pihak ketiga) */
        ...
        @media (prefers-reduced-motion: reduce) {
            .auth-bg-base, .auth-bg-blur, .auth-main, .bridge-loader, .premium-alert, .premium-alert-backdrop,
            .brand-side, .form-side { transition: none !important; }
            .fluent-spinner { animation-duration: 1s !important; }
        }
    </style>
    @stack('styles')
</head>
<body>
    <!-- Sprite SVG tunggal — dipakai ulang oleh semua ikon di seluruh halaman lewat <use> -->
    <svg class="icon-sprite" aria-hidden="true">
        <symbol id="icon-user-group" viewBox="0 0 24 24">
            ...
            <path d="M15.5 20c.3-2.5 2.1-4.4 4.5-4.8"></path>
        </symbol>
        ...
        <symbol id="icon-chart" viewBox="0 0 24 24">
            <rect x="4" y="12" width="3.4" height="8" rx="1"></rect>
            <rect x="10.3" y="7" width="3.4" height="13" rx="1"></rect>
            <rect x="16.6" y="3" width="3.4" height="17" rx="1"></rect>
        </symbol>
        <symbol id="icon-heart-pulse" viewBox="0 0 24 24">
            <path d="M12 20.5S3.5 15 3.5 9a4.5 4.5 0 0 1 8-2.8A4.5 4.5 0 0 1 20.5 9c0 6-8.5 11.5-8.5 11.5z"></path>
            <path d="M6.2 11h2.4l1.4-2.8 2 4 1.3-1.2H17.8"></path>
        </symbol>
        ...
        <symbol id="icon-alert-triangle" viewBox="0 0 24 24">
            <path d="M12 3.5L21.5 20h-19L12 3.5z"></path>
            <line x1="12" y1="9.5" x2="12" y2="13.5"></line>
            <circle cx="12" cy="16.6" r="0.9" fill="currentColor" stroke="none"></circle>
        </symbol>
    ...
    <!-- Premium Alert: pengganti SweetAlert2, ringan (tanpa JS/CSS pihak ketiga), API tetap sama: window.showAuthAlert(title, html, type) -->
