    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        ::view-transition-old(root) {
            animation: authViewOut .25s cubic-bezier(0.22, 1, 0.36, 1) both;
        }
        ::view-transition-new(root) {
            animation: authViewIn .3s cubic-bezier(0.22, 1, 0.36, 1) both;
        }
        @keyframes authViewOut { to { opacity: 0; transform: scale(0.985); } }
        @keyframes authViewIn { from { opacity: 0; transform: scale(1.015); } }

        body { animation: pageFadeIn .15s cubic-bezier(0.22, 1, 0.36, 1) both; }
        ...
        /* PREMIUM ALERT (SQUIRCLE & ULTRA CLEAN) */
        ...
        .premium-alert {
            ...
            box-shadow: 0 35px 75px -15px rgba(2, 6, 23, 0.3), 0 0 0 1px rgba(255,255,255,0.5) inset;
            text-align: center;
        ...
        .premium-alert.is-open .premium-alert-icon-wrap {
            animation: alertIconPop 0.42s cubic-bezier(0.34, 1.56, 0.64, 1) 0.06s both;
        }
        @keyframes alertIconPop { to { transform: scale(1); opacity: 1; } }
        .premium-alert-icon-wrap .icon { width: 32px; height: 32px; }
        .premium-alert-icon-wrap.is-success { background: #ecfdf5; color: var(--green-600); box-shadow: 0 0 0 10px rgba(16,185,129,0.08), 0 12px 24px -6px rgba(16,185,129,0.3); }
        .premium-alert-icon-wrap.is-error   { background: #fef2f2; color: #dc2626; box-shadow: 0 0 0 10px rgba(220,38,38,0.08), 0 12px 24px -6px rgba(220,38,38,0.3); }
        .premium-alert-icon-wrap.is-warning { background: #fffbeb; color: var(--amber-500); box-shadow: 0 0 0 10px rgba(245,158,11,0.08), 0 12px 24px -6px rgba(245,158,11,0.3); }
        .premium-alert-icon-wrap.is-info    { background: #eff6ff; color: #2563eb; box-shadow: 0 0 0 10px rgba(37,99,235,0.08), 0 12px 24px -6px rgba(37,99,235,0.3); }

        .premium-alert-title { color: var(--slate-900); font-size: 1.25rem; font-weight: 800; margin: 0 0 8px; letter-spacing: -0.03em; }
        .premium-alert-msg { color: var(--slate-600); font-size: 0.92rem; font-weight: 500; line-height: 1.6; margin: 0; }
        .premium-alert-msg b { color: var(--slate-900); }

        .premium-alert-confirm {
            border: none; border-radius: 1rem; background: var(--slate-900); color: #fff;
            font-weight: 700; font-size: 0.9rem; padding: 14px 24px; width: 100%;
            margin-top: 24px; cursor: pointer; transition: transform 0.12s ease-out, background 0.15s ease, box-shadow 0.15s ease;
            font-family: inherit;
        }
        ...
        /* GERBANG TRANSISI (TANPA GARIS TENGAH / SEAMLESS OVERLAP) */
        .gate-overlay { position: fixed; inset: 0; z-index: 3000; pointer-events: none; overflow: hidden; }

        .gate-panel {
            position: absolute; top: 0; bottom: 0; width: calc(50.5%);
            background: linear-gradient(160deg, #10b981 0%, #0f766e 40%, #134e4a 70%, #0a1f1c 100%);
            will-change: transform, filter;
            transition: transform .45s cubic-bezier(0.77, 0, 0.175, 1), filter .45s ease;
        }
        .gate-left  { left: -0.5%; }
        .gate-right { right: -0.5%; transition-delay: .015s; }
        .gate-overlay.is-open .gate-left  { transform: translateX(-102%) scale(1.02); filter: blur(1px); }
        .gate-overlay.is-open .gate-right { transform: translateX(102%) scale(1.02); filter: blur(1px); }

        .gate-brand {
            position: absolute; inset: 0; display: flex; flex-direction: column;
            align-items: center; justify-content: center; gap: 18px;
            opacity: 1; transform: scale(1) translateY(0); filter: blur(0);
            transition: opacity .2s ease, transform .28s cubic-bezier(0.34, 1.56, 0.64, 1), filter .2s ease;
        }
        .gate-overlay.is-open .gate-brand { opacity: 0; transform: scale(0.94) translateY(-4px); filter: blur(3px); }

        .gate-logo-wrap { position: relative; width: 140px; display: flex; align-items: center; justify-content: center; }
        .gate-logo-wrap::before {
            content: ''; position: absolute; inset: -18px -30px; border-radius: 50%;
            background: radial-gradient(circle, rgba(255,255,255,0.2), transparent 70%);
            animation: gateGlow 2s ease-in-out infinite;
        }
        @keyframes gateGlow { 0%, 100% { opacity: .5; transform: scale(0.92); } 50% { opacity: 1; transform: scale(1.06); } }
        .gate-logo { position: relative; width: 100%; height: auto; filter: brightness(0) invert(1); opacity: .98; animation: gateFloat 2.8s ease-in-out infinite; }
        @keyframes gateFloat { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-3px); } }

        .gate-bar { width: 110px; height: 3px; border-radius: 999px; background: rgba(255,255,255,0.18); overflow: hidden; margin-top: 4px; }
        .gate-bar span {
            display: block; width: 45%; height: 100%; border-radius: 999px;
            background: linear-gradient(90deg, transparent, #fff, transparent);
            animation: gateBarSweep 0.8s cubic-bezier(0.4,0,0.2,1) infinite;
        }
        ...
        @media (prefers-reduced-motion: reduce) {
            ...
            .gate-logo, .gate-logo-wrap::before, .gate-bar span { animation: none !important; }
        }
    </style>
    ...
    <script>
        (function () {
            ...
            function openGate() {
                ...
                setTimeout(function () { gate.style.visibility = 'hidden'; }, 450);
            }

            var MIN_CLOSED_MS = 100, MAX_WAIT_MS = 600;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            @endif
            @if(session('error'))
                window.showAuthAlert('Otentikasi Gagal', @json(session('error')), 'error');
            @endif
        });
    </script>
    @stack('scripts')
