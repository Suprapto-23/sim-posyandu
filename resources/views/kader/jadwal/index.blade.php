...
@php
    ...
    $hakAkses = $hakAkses ?? ['semua'];
    $jadwalCards = collect($jadwalCards ?? []);

    $indexRoute = Route::has('user.jadwal.index')
...
<style>
    ...
    .jd-enter {
        opacity: 0;
        animation: jdEnter .36s cubic-bezier(.16,1,.3,1) forwards;
    }

    .jd-enter-2 {
        opacity: 0;
        animation: jdEnter .36s cubic-bezier(.16,1,.3,1) .07s forwards;
    }
...
@section('content')
<div class="jd-page -mx-4 -my-4 min-h-[calc(100vh-96px)] px-4 py-5 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
    <div class="mx-auto max-w-7xl space-y-5">

        <section class="jd-enter jd-glass relative overflow-hidden rounded-[30px] p-5 sm:p-6">
            ...
            <div class="relative grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_420px] xl:items-center">
                <div>
                    ...
                    <p class="mt-2 max-w-3xl text-sm font-semibold leading-7 text-slate-600">
                        Daftar agenda Posyandu yang relevan dengan akun warga dan data keluarga terhubung.
                    </p>
                    ...
                    <div class="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
                        <div class="rounded-[20px] border border-emerald-100 bg-white/70 px-3 py-3">
                            ...
                            <p class="mt-1 text-2xl font-black text-slate-800">{{ $summary['semua'] }}</p>
                        </div>
                        ...
                        <div class="rounded-[20px] border border-rose-100 bg-white/70 px-3 py-3">
                            ...
                            <p class="mt-1 text-2xl font-black text-rose-600">{{ $summary['balita'] }}</p>
                        </div>
                        ...
                        <div class="rounded-[20px] border border-sky-100 bg-white/70 px-3 py-3">
                            ...
                            <p class="mt-1 text-2xl font-black text-sky-600">{{ $summary['remaja'] }}</p>
                        </div>
                        ...
                        <div class="rounded-[20px] border border-amber-100 bg-white/70 px-3 py-3">
                            ...
                            <p class="mt-1 text-2xl font-black text-amber-600">{{ $summary['lansia'] }}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="mt-3 grid grid-cols-2 gap-3">
                        <select name="filter"
                            ...
                            @foreach($filterOptions as $key => $label)
                                @if($key === 'semua' || in_array($key, $hakAkses))
                                    <option value="{{ $key }}" @selected($filters['filter'] === $key)>
                                        {{ $label }}
                                    </option>
                                @endif
                            @endforeach
        ...
        <section class="jd-enter-2 grid grid-cols-1 gap-4 xl:grid-cols-2">
            @forelse($jadwalCards as $card)
                @php
                    $targetTone = $toneMap[$card['target_tone']] ?? $toneMap['emerald'];
                    $statusTone = $toneMap[$card['status_tone']] ?? $toneMap['slate'];
                @endphp

                <article class="jd-card group relative overflow-hidden rounded-[28px] p-4 sm:p-5">
                    <div class="absolute left-0 top-0 h-full w-1.5 {{ $targetTone['line'] }}"></div>

                    <div class="flex flex-col gap-4 sm:flex-row">
                        <div class="flex h-20 w-20 shrink-0 flex-col items-center justify-center rounded-[24px] border border-emerald-100 bg-white/78 text-emerald-700 shadow-sm">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap gap-2">
                                <span class="rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.10em] {{ $targetTone['badge'] }}">
                                    {{ $card['target_label'] }}
                                </span>

                                <span class="rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.10em] {{ $statusTone['badge'] }}">
                                    {{ $card['status_label'] }}
                                </span>
                            </div>
                            ...
                            <div class="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-3">
                                ...
                                <span class="inline-flex h-9 items-center justify-center rounded-2xl border border-emerald-100 bg-white/82 px-4 text-xs font-black uppercase tracking-[0.12em] text-emerald-700">
                                    Dikelola Bidan
                                </span>
                            </div>
                        </div>
            ...
            @empty
                <div class="xl:col-span-2 rounded-[28px] border border-dashed border-emerald-300 bg-emerald-50/65 p-10 text-center">
                    ...
                    <p class="mx-auto mt-2 max-w-md text-sm font-semibold leading-6 text-slate-600">
                        Belum ada jadwal Posyandu yang sesuai dengan filter saat ini.
                    </p>

                    <a href="{{ $indexRoute }}"
                       class="mt-5 inline-flex h-11 items-center justify-center rounded-2xl bg-emerald-600 px-5 text-xs font-black uppercase tracking-[0.12em] text-white shadow-[0_12px_26px_rgba(16,185,129,.22)] transition hover:bg-emerald-700">
                        Reset Filter
                    </a>
                </div>
            @endforelse
        ...
        @if(method_exists($jadwalKegiatan, 'links') && $jadwalKegiatan->hasPages())
            <section class="jd-enter-2 rounded-[24px] border border-white/75 bg-white/70 px-4 py-4 shadow-sm backdrop-blur-xl">
