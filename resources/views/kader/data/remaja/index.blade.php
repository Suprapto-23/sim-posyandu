...
<div class="max-w-[1280px] mx-auto animate-pop-in pb-20 px-4 sm:px-6 lg:px-8 mt-6 relative">
    ...
    <section class="grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4 mb-6 sm:mb-8">
        ...
        <div class="widget-card p-4 sm:p-6 flex justify-between items-center group hover:-translate-y-1 transition-transform duration-300 min-w-0">
            <div class="min-w-0">
                <p class="text-[9px] sm:text-[11px] font-bold uppercase tracking-wider text-emerald-600 truncate">Terhubung Akun</p>
    ...
    {{-- MAIN DATA WIDGET --}}
    <section id="remajaLiveRegion" data-live-region class="w-full widget-card p-4 sm:p-6">
        
        <div class="pb-4 mb-4 border-b border-slate-100 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
                <h2 class="text-base sm:text-xl font-black text-slate-900">Daftar Sasaran Remaja</h2>
            ...
            <div class="flex flex-row gap-2 sm:gap-3">
                <label class="btn-pill flex-1 sm:flex-none border border-slate-200 bg-white/50 px-4 py-2 sm:py-2.5 text-[10px] sm:text-[11px] font-black uppercase tracking-wider text-slate-600 shadow-sm flex items-center justify-center gap-2 cursor-pointer hover:bg-slate-50 transition-colors">
                    <input type="checkbox" id="selectAllRemaja" class="h-4 w-4 rounded border-slate-300 text-teal-500 focus:ring-teal-400 cursor-pointer transition">
                    Pilih Semua
                </label>
        ...
        <div class="pc-scroll-container max-h-[60vh] lg:max-h-[600px] py-2 space-y-3">
            @forelse($items as $item)
                ...
                {{-- STRUKTUR FLEXBOX BARU UNTUK KARTU --}}
                    ...
                    <div class="flex flex-col xl:flex-row gap-4 xl:items-center">
                        ...
                        <div class="flex items-start gap-3 sm:gap-4 xl:w-[35%] shrink-0">
                            <div class="mt-1 xl:mt-0">
                                <input type="checkbox" name="ids[]" value="{{ $item->id }}" form="bulkDeleteForm" class="remaja-check h-4 w-4 sm:h-5 sm:w-5 rounded border-slate-300 text-teal-600 focus:ring-teal-500 cursor-pointer">
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-3 xl:flex xl:w-[45%] shrink-0 xl:gap-4 border-t border-slate-100 pt-3 xl:border-0 xl:pt-0">
                            <div class="bg-slate-50/50 p-2.5 sm:p-3 rounded-2xl border border-slate-100 xl:flex-1">
                                <p class="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Pendidikan & Usia</p>
                                <p class="text-[11px] sm:text-xs font-bold text-slate-700 leading-snug">{{ $item->sekolah ?: '-' }}</p>
                                <p class="text-[10px] sm:text-[11px] font-semibold text-slate-500 mt-0.5 truncate" title="{{ $getAge($item) }}"><i class="fa-solid fa-clock opacity-50 mr-1"></i> {{ $getAge($item) }}</p>
                            </div>
                            <div class="bg-slate-50/50 p-2.5 sm:p-3 rounded-2xl border border-slate-100 xl:flex-1">
                                <p class="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Orang Tua / Wali</p>
                                <p class="text-[11px] sm:text-xs font-bold text-slate-700 truncate" title="{{ $item->nama_ortu }}">Wali: {{ $item->nama_ortu ?: '-' }}</p>
                                <p class="text-[10px] sm:text-[11px] font-semibold text-slate-500 mt-0.5 line-clamp-1 leading-snug" title="{{ $item->alamat }}"><i class="fa-solid fa-location-dot opacity-50 mr-1"></i> {{ $item->alamat ?: '-' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap xl:flex-nowrap gap-2 pt-3 border-t border-slate-100 xl:border-0 xl:pt-0 xl:w-[20%] xl:justify-end shrink-0">
                            @if($routeHas('kader.data.remaja.show'))
                                <a href="{{ route('kader.data.remaja.show', $item->id) }}" class="btn-pill flex-1 xl:flex-none w-auto xl:w-10 h-10 flex items-center justify-center border border-slate-200 bg-white text-[11px] sm:text-xs font-bold text-slate-600 shadow-sm hover:border-teal-300 hover:text-teal-600 hover:bg-teal-50 transition-colors" title="Lihat Detail">
                            ...
                            @if($routeHas('kader.data.remaja.edit'))
                                <a href="{{ route('kader.data.remaja.edit', $item->id) }}" class="btn-pill flex-1 xl:flex-none w-auto xl:w-10 h-10 flex items-center justify-center border border-slate-200 bg-white text-[11px] sm:text-xs font-bold text-slate-600 shadow-sm hover:border-amber-300 hover:text-amber-600 hover:bg-amber-50 transition-colors" title="Edit Data">
                                    <i class="fa-solid fa-pen"></i> <span class="xl:hidden ml-1.5 uppercase tracking-wider">Edit</span>
                                </a>
                            @endif

                            @if(! $connected)
                                @php $syncUrl = $routeHas('kader.data.remaja.sync') ? route('kader.data.remaja.sync', $item->id) : url('/kader/data/remaja/' . $item->id . '/sync'); @endphp
                                <button type="button" 
            ...
            @empty
                <div class="rounded-[2rem] border-2 border-dashed border-slate-200 bg-white/50 p-8 sm:p-12 text-center transition-all">
                    ...
                    <h3 class="text-lg sm:text-xl font-black text-slate-800">Data Remaja Kosong</h3>
        ...
        @if(method_exists($items, 'hasPages') && $items->hasPages())
            <div id="remajaPagination" class="pt-5 sm:pt-6 mt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
