...
<div class="max-w-[1280px] mx-auto animate-pop-in pb-20 px-4 sm:px-6 lg:px-8 mt-6 relative">
    ...
    {{-- FILTER WIDGET (DISAMAKAN PERSIS DENGAN BALITA) --}}
    ...
    {{-- MAIN DATA WIDGET --}}
    <section id="remajaLiveRegion" data-live-region class="w-full">
        ...
        <div class="px-2 py-2 mb-4 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                <label class="btn-pill border border-slate-200 bg-white px-4 py-2 text-[11px] font-black uppercase tracking-wider text-slate-600 shadow-sm flex items-center justify-center gap-2 cursor-pointer hover:bg-slate-50 transition-colors">
                    <input type="checkbox" id="selectAllRemaja" class="h-4 w-4 rounded border-slate-300 text-emerald-500 focus:ring-emerald-400 cursor-pointer transition">
                    Pilih Semua
                </label>
            ...
            @forelse($items as $item)
                ...
                <article class="bg-white border border-slate-100 rounded-[1.25rem] p-4 shadow-sm hover:shadow-md hover:border-emerald-200 transition duration-300 transform translate-z-0">
                    <div class="data-row col-stack-mobile">
                        ...
                        <div class="col-hidden-mobile min-w-0 relative z-10 mt-1 xl:mt-0">
                            <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Pendidikan & Usia</p>
                            <p class="text-[13px] font-bold text-slate-700 leading-snug">{{ $item->sekolah ?: '-' }}</p>
                            <p class="text-[11px] font-semibold text-slate-500 mt-1" title="{{ $getAge($item) }}">{{ $getAge($item) }}</p>
                        </div>

                        <div class="col-hidden-mobile min-w-0 relative z-10 mt-1 xl:mt-0">
                            <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Orang Tua / Wali</p>
                            <p class="text-[13px] font-bold text-slate-700 truncate" title="{{ $item->nama_ortu }}">Wali: {{ $item->nama_ortu ?: '-' }}</p>
                        ...
                        <div class="action-box relative z-20">
                            @if($routeHas('kader.data.remaja.show'))
                                <a href="{{ route('kader.data.remaja.show', $item->id) }}" class="row-action action-detail" title="Lihat Detail"><i class="fa-solid fa-eye"></i></a>
                            @endif

                            @if($routeHas('kader.data.remaja.edit'))
                                <a href="{{ route('kader.data.remaja.edit', $item->id) }}" class="row-action action-edit" title="Edit Data"><i class="fa-solid fa-pen"></i></a>
                            @endif

                            @if(! $connected)
                                @php
                                    $syncUrl = $routeHas('kader.data.remaja.sync') ? route('kader.data.remaja.sync', $item->id) : url('/kader/data/remaja/' . $item->id . '/sync');
                                @endphp
                                <button type="button" 
        ...
        @if(method_exists($items, 'hasPages') && $items->hasPages())
            <div id="remajaPagination" class="px-2 py-6 mt-2 flex flex-col sm:flex-row items-center justify-between gap-4">
