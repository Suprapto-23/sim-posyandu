...
@php
    ...
    // Helper Ekstraksi Data Pemeriksaan Balita
    ...
    $lastWeight = fn($b) => $cleanNum($b->pemeriksaan_terakhir->berat_badan ?? null);
    $lastHeight = fn($b) => $cleanNum($b->pemeriksaan_terakhir->tinggi_badan ?? null);
...
<style>
    ...
    /* Grid Layout - Rapi ala Remaja/Lansia */
    .data-row {
        display: grid;
...
<div class="max-w-[1280px] mx-auto animate-pop-in pb-20 px-4 sm:px-6 lg:px-8 mt-6">
    ...
    <section class="bg-gradient-to-br from-emerald-500 via-teal-500 to-teal-600 rounded-[3rem] p-8 md:p-10 mb-8 relative overflow-hidden shadow-[0_20px_40px_-12px_rgba(16,185,129,.35)] border border-white/20">
        ...
        <div class="relative z-10 flex flex-col lg:flex-row gap-8 lg:items-center justify-between">
            ...
            <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto shrink-0 justify-center">
                {{-- HTML5 NATIVE DOWNLOAD: Sangat Instan, tanpa JS, tanpa tab baru --}}
                @if($templateRoute)
                    <a href="{{ $templateRoute }}" download="Template_Data_Balita.xlsx" class="btn-pill bg-white/20 hover:bg-white/30 text-white border border-white/30 px-6 py-3.5 text-sm font-bold backdrop-blur-md flex items-center justify-center gap-2 transition-all shadow-sm">
    ...
    {{-- FILTER WIDGET --}}
    <form id="liveSearchForm" action="{{ $indexRoute }}" method="GET" class="widget-card p-4 sm:p-6 flex flex-col lg:flex-row gap-4 items-center z-20 relative mb-8">
        ...
        <select id="status_akun" name="status_akun" class="w-full lg:w-48 btn-pill border border-slate-200 bg-white/80 px-4 py-3.5 text-sm font-bold text-slate-700 outline-none transition focus:bg-white focus:border-emerald-400 shadow-inner appearance-none cursor-pointer">
    ...
    <section id="balitaLiveRegion" data-live-region class="widget-card overflow-hidden">
        ...
        <div class="border-b border-slate-100 bg-slate-50/50 px-6 py-5 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                <label class="btn-pill border border-slate-200 bg-white px-4 py-2 text-[11px] font-black uppercase tracking-wider text-slate-600 shadow-sm flex items-center justify-center gap-2 cursor-pointer hover:bg-slate-50">
                    <input type="checkbox" id="selectAllBalita" class="h-4 w-4 rounded border-slate-300 text-emerald-500 focus:ring-emerald-400 cursor-pointer">
                    Pilih Semua
                </label>
            ...
            @forelse($items as $item)
                ...
                <article class="bg-white border border-emerald-100/70 rounded-[1.5rem] p-4 shadow-sm hover:shadow-md hover:border-emerald-300 transition duration-300">
                    <div class="data-row col-stack-mobile">
                        ...
                        {{-- 2. Identitas (Desain Compact Padat Ala Remaja/Lansia) --}}
                        ...
                        <div class="col-hidden-mobile min-w-0 relative z-10 mt-1">
                            <p class="text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">Usia & TTL</p>
                            <p class="text-xs font-black text-slate-800 leading-snug" title="{{ $getAge($item) }}">{{ $getAge($item) }}</p>
                            <p class="text-[11px] font-bold text-slate-500 mt-1">{{ $formatDate($item->tanggal_lahir) }}</p>
                        </div>

                        {{-- 4. Orang Tua & Domisili (Line Clamp) --}}
                        <div class="col-hidden-mobile min-w-0 relative z-10 mt-1">
                            <p class="text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">Orang Tua / Wali</p>
                            <p class="text-xs font-black text-slate-800" title="{{ $item->nama_ibu }}">Ibu: {{ $item->nama_ibu ?: '-' }}</p>
                            <p class="text-[11px] font-bold text-slate-500 mt-1 line-clamp-2 leading-snug" title="{{ $item->alamat }}">{{ $item->alamat ?: '-' }}</p>
                        </div>

                        {{-- 5. Cek Terakhir Box (Background Soft) --}}
                        ...
                        {{-- 6. Action Box (MURNI EVENT DELEGATION JS - 100% ANTI BUG) --}}
                        <div class="action-box relative z-20 mt-1">
                            ...
                            @if(! $connected && $routeHas('kader.data.balita.sync'))
                                <button type="button" 
                                        ...
                                        data-confirm-submit
                                        data-action-url="{{ route('kader.data.balita.sync', $item->id) }}"
        ...
        @if(method_exists($items, 'hasPages') && $items->hasPages())
            <div id="balitaPagination" class="border-t border-slate-100 bg-slate-50/50 px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-4">
    ...
    {{-- GLOBAL ACTION FORM (Dipakai otomatis oleh JS untuk action Delete/Sync pada baris) --}}
