...
@php
    ...
    // Helper Pemeriksaan (Anti Error / Null Safe)
        ...
        <div class="relative z-10 flex flex-col items-center">
            ...
            <div class="mt-4 flex flex-wrap items-center justify-center gap-3">
                <span class="bg-white/20 backdrop-blur-md border border-white/30 text-white font-bold text-[11px] uppercase tracking-widest px-4 py-1.5 rounded-full shadow-sm">
                    <i class="fas fa-person-cane mr-1"></i> Sasaran Lansia
                </span>
                ...
                <span class="bg-white/20 backdrop-blur-md border border-white/30 text-white font-mono text-[11px] font-bold px-4 py-1.5 rounded-full flex items-center gap-2 shadow-sm">
                    <i class="fas fa-id-card"></i> NIK: {{ $lansia->nik ?? '-' }}
                </span>
    ...
    <section class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div class="soft-card bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6 sm:p-8">
            ...
            <div class="space-y-4">
                <div class="info-row">
                    ...
                    <span class="text-[13px] font-black text-slate-700">{{ $lansia->nama_lengkap ?? '-' }}</span>
                </div>
                ...
                <div class="info-row">
                    ...
                    <span class="text-[13px] font-black text-slate-700">{{ $lansia->tempat_lahir ?? '-' }}</span>
                </div>
                ...
                <div class="info-row border-t border-slate-100 pt-4">
                    <span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Pekerjaan / Status</span>
                    <span class="text-[13px] font-black text-slate-700">{{ $lansia->pekerjaan ?: '-' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="soft-card bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6 sm:p-8 flex-1">
                ...
                <div class="space-y-4">
                    <div class="info-row">
                        <span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Pendamping / Keluarga</span>
                        <span class="text-[13px] font-black text-slate-700">{{ $lansia->nama_keluarga ?: '-' }}</span>
                    </div>

                    <div class="info-row">
                        <span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Nomor Telepon</span>
                        <span class="text-[13px] font-black text-slate-700">{{ $lansia->telepon_keluarga ?: '-' }}</span>
                    </div>
                    ...
                    <div class="flex flex-col border-b border-dashed border-slate-200 pb-3 gap-1">
                        ...
                        <span class="text-[13px] font-black text-slate-700 leading-relaxed">{{ $lansia->alamat ?: '-' }}</span>
                    </div>
            ...
            <div class="soft-card bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6 sm:p-8">
                ...
                @if($akunTerhubung)
                    ...
                    <p class="text-xs font-semibold text-slate-500 mt-3">Keluarga atau pendamping dapat memantau riwayat pemeriksaan melalui aplikasi warga.</p>
                @else
                    ...
                    <p class="text-xs font-semibold text-slate-500 mt-3 mb-4">Sinkronkan data agar keluarga dapat mengakses riwayat kesehatan Lansia secara mandiri.</p>
                    
                    @if($syncRoute)
                        <form id="syncLansiaForm" action="{{ $syncRoute }}" method="POST">
                            @csrf
    ...
    <section class="soft-card bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6 sm:p-8">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 border-b border-slate-50 pb-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h4 class="text-sm font-black text-slate-800 uppercase tracking-widest">Riwayat Pemeriksaan</h4>
                    <p class="text-[11px] font-bold text-slate-400">5 Pemeriksaan Terakhir</p>
                </div>
            </div>
        ...
        <div class="space-y-3">
            @forelse($kunjungans as $kunjungan)
                @php
                    ...
                    // Ekstraksi data spesifik Lansia (Aman dari null error)
                    $pmr = $kunjungan->pemeriksaan ?? null;
                    $lBerat = $cleanNum($pmr->berat_badan ?? null);
                    $lTinggi = $cleanNum($pmr->tinggi_badan ?? null);
                    $lLp = $cleanNum($pmr->lingkar_perut ?? null);
                    $lTensi = ($pmr->tekanan_darah_sistolik ?? $pmr->tekanan_darah ?? '-');
                    
                    // Metrik tambahan lansia
                    $lGula = $cleanNum($pmr->gula_darah ?? null);
                    $lAsamUrat = $cleanNum($pmr->asam_urat ?? null);
                    $lKolesterol = $cleanNum($pmr->kolesterol ?? null);
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                ...
                                <span class="inline-flex w-max rounded-full bg-white px-3 py-1 text-[10px] font-black uppercase tracking-widest text-slate-600 border border-slate-200 shadow-sm">
                                    <i class="fas fa-user-nurse mr-1.5 text-teal-500"></i> Kader {{ Str::limit($petugas, 15) }}
                                </span>
                            </div>

                            <div class="mt-3 bg-white rounded-xl border border-slate-100 p-3">
                                @if($pmr)
                                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 text-[11px] font-bold text-slate-700">
                                        <div class="flex items-center gap-1.5 border-r border-slate-100 pr-2">
                                            <i class="fa-solid fa-weight-scale text-slate-400 w-4 text-center"></i>
                                            <span class="text-emerald-600">{{ $lBerat }} <span class="text-[9px] text-slate-400">kg</span></span>
                                        </div>
                                        <div class="flex items-center gap-1.5 lg:border-r border-slate-100 pr-2">
                                            <i class="fa-solid fa-ruler-vertical text-slate-400 w-4 text-center"></i>
                                            <span class="text-sky-600">{{ $lTinggi }} <span class="text-[9px] text-slate-400">cm</span></span>
                                        </div>
                                        <div class="flex items-center gap-1.5 border-r border-slate-100 pr-2">
                                            <i class="fa-solid fa-tape text-slate-400 w-4 text-center"></i>
                                            <span class="text-indigo-500">{{ $lLp }} <span class="text-[9px] text-slate-400">cm</span></span>
                                        </div>
                                        <div class="flex items-center gap-1.5 pr-2">
                                            <i class="fa-solid fa-heart-pulse text-slate-400 w-4 text-center"></i>
                                            <span class="text-rose-500">{{ $lTensi }}</span>
                                        </div>
                                        
                                        <div class="col-span-2 lg:col-span-4 mt-1 border-t border-slate-100 pt-2 grid grid-cols-3 gap-2">
                                            <div class="flex items-center gap-1.5 border-r border-slate-100 pr-2" title="Gula Darah">
                                                <i class="fa-solid fa-vial text-amber-400 w-4 text-center"></i>
                                                Gula: <span class="text-amber-600">{{ $lGula }} <span class="text-[9px] text-slate-400">mg/dL</span></span>
                                            </div>
                                            <div class="flex items-center gap-1.5 border-r border-slate-100 pr-2" title="Asam Urat">
                                                <i class="fa-solid fa-flask text-purple-400 w-4 text-center"></i>
                                                Urat: <span class="text-purple-600">{{ $lAsamUrat }} <span class="text-[9px] text-slate-400">mg/dL</span></span>
                                            </div>
                                            <div class="flex items-center gap-1.5 pr-2" title="Kolesterol">
                                                <i class="fa-solid fa-bacon text-orange-400 w-4 text-center"></i>
                                                Kolest: <span class="text-orange-600">{{ $lKolesterol }} <span class="text-[9px] text-slate-400">mg/dL</span></span>
                                            </div>
                                        </div>
                                    </div>
                                @else
                                    <p class="text-xs font-medium text-slate-500 italic">Tidak ada pencatatan metrik pada kunjungan ini.</p>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            @empty
                <div class="rounded-2xl border border-dashed border-slate-200 bg-slate-50/50 p-8 text-center">
                    ...
                    <p class="mx-auto mt-1 max-w-sm text-xs font-semibold leading-relaxed text-slate-500">
                        Belum ada catatan kunjungan atau pemeriksaan fisik untuk Lansia ini.
                    </p>
                </div>
            @endforelse
        </div>
    ...
    <div class="pc-modal-card">
        <div class="relative overflow-hidden bg-gradient-to-br from-amber-500 to-orange-600 px-6 py-6 text-white text-center">
            ...
            <div class="relative z-10">
                ...
                <p class="text-xs font-semibold leading-relaxed opacity-90 px-4">
                    Sistem akan mencari akun keluarga dengan NIK yang sama dengan NIK Lansia ini.
                </p>
            </div>
        </div>
        ...
        <div class="p-6 bg-white text-center">
            <div class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs font-bold leading-6 text-amber-800 mb-5 text-left">
                <i class="fa-solid fa-circle-info mr-1"></i> Sinkronisasi otomatis berhasil jika akun keluarga sudah terdaftar di sistem dengan NIK yang sesuai.
            </div>
