...
<style>
    ...
    .pc-choice-btn.btn-present-active {
        ...
        box-shadow: 0 0 20px rgba(16, 185, 129, 0.6), inset 0 -2px 0 rgba(0,0,0,0.1) !important;
    ...
    .pc-choice-btn.btn-absent-active {
        ...
        box-shadow: 0 0 20px rgba(245, 158, 11, 0.6), inset 0 -2px 0 rgba(0,0,0,0.1) !important;
        transform: scale(1.05);
        font-weight: 800;
        z-index: 10;
    }
    ...
    .nexus-modal-content { 
        ...
        box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25), 0 0 40px rgba(245, 158, 11, 0.15);
    }
...
@section('content')
<div id="kaderAbsensiPage" class="px-4 py-8 sm:px-6 lg:px-8 max-w-[1400px] mx-auto space-y-6 relative">

    {{-- SYSTEM / VALIDATION ERROR ALERT --}}
    @if(session('error') || $errors->any())
    <div class="rounded-[2rem] bg-rose-50 border-2 border-rose-200 p-6 shadow-lg shadow-rose-100/50 flex flex-col sm:flex-row items-center sm:items-start gap-4 relative overflow-hidden">
        ...
        <div>
            <h3 class="font-black text-rose-800 text-lg">Gagal Menyimpan Data</h3>
            @if(session('error'))
                <p class="text-rose-600 font-medium text-sm mt-1">{{ session('error') }}</p>
            @endif
            @if($errors->any())
                <ul class="text-rose-600 font-medium text-sm mt-1 list-disc list-inside">
    ...
    <div class="relative overflow-hidden rounded-[3rem] bg-gradient-to-r from-teal-500 via-teal-400 to-emerald-400 p-6 sm:p-10 shadow-2xl shadow-teal-500/30 flex flex-col xl:flex-row justify-between items-center gap-8 border-[4px] border-white/50">
        ...
        <div class="relative z-10 w-full xl:w-1/2 flex flex-col gap-4 text-center xl:text-left">
            ...
            <div class="flex flex-wrap justify-center xl:justify-start gap-3 mt-2">
                @if($routeHas('kader.absensi.riwayat'))
                    <a href="{{ route('kader.absensi.riwayat', ['kategori' => $kategoriAktif]) }}" class="btn-pill bg-white text-teal-600 hover:text-teal-800 hover:bg-teal-50 px-6 py-3 text-sm font-bold shadow-[0_8px_20px_rgba(255,255,255,0.3)] flex items-center gap-2">
        ...
        <div class="relative z-10 w-full xl:w-auto flex flex-col sm:flex-row gap-4 justify-center">
            <div class="widget-card !rounded-[2.5rem] !bg-white/90 p-5 flex items-center gap-4 min-w-[220px]">
                ...
                <div>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Sesi Absensi</p>
                    <p class="text-sm font-black text-slate-800">{{ $tanggalFormat }}</p>
                </div>
            </div>

            <div class="widget-card !rounded-[2.5rem] !bg-white/90 p-5 flex items-center gap-4 min-w-[220px]">
                <div class="w-12 h-12 rounded-full {{ $toneClass($currentKategori['tone'], 'light') }} flex items-center justify-center text-xl shadow-inner shrink-0">
                    <i class="fa-solid {{ $currentKategori['icon'] }}"></i>
                </div>
                <div>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Kategori</p>
                    <p class="text-sm font-black text-slate-800">{{ $currentKategori['label'] }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div class="widget-card p-6 flex justify-between items-center group hover:-translate-y-1">
            <div>
                ...
                <p class="mt-1 text-3xl font-black text-slate-800">{{ number_format($totalSasaran) }}</p>
            </div>
        ...
        <div class="widget-card p-6 flex justify-between items-center group hover:-translate-y-1">
            <div>
                ...
                <p class="mt-1 text-3xl font-black text-slate-800" id="hadirCountText">{{ number_format($totalHadir) }}</p>
            </div>
        ...
        <div class="widget-card p-6 flex justify-between items-center group hover:-translate-y-1">
            <div>
                ...
                <p class="mt-1 text-3xl font-black text-slate-800" id="tidakCountText">{{ number_format($totalTidakHadir) }}</p>
            </div>
        ...
        <div class="widget-card p-6 flex justify-between items-center group hover:-translate-y-1">
            <div>
                <p class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Belum Dipilih</p>
                <p class="mt-1 text-3xl font-black text-slate-800" id="belumCountText">{{ number_format($belumTercatat) }}</p>
            </div>
            <div class="w-14 h-14 rounded-2xl bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-400 -rotate-3 group-hover:-rotate-6 transition-transform">
    ...
    {{-- 3. FILTER WIDGETS --}}
    <section class="grid items-stretch gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div class="widget-card p-6 flex flex-col justify-center">
            <p class="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-4">Pilih Kategori Sasaran</p>
            <div class="grid gap-3 sm:grid-cols-3">
                @foreach($kategoriMenus as $key => $item)
                    ...
                    @endphp
                    <a href="{{ $url }}" class="btn-pill border p-3 flex items-center gap-3 transition-all {{ $active ? 'border-teal-500 bg-teal-500 text-white shadow-[0_5px_15px_rgba(20,184,166,0.4)] transform scale-[1.02]' : 'border-slate-100 bg-white/50 hover:bg-white hover:border-teal-300' }}">
                        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full {{ $active ? 'bg-white/20 text-white' : $toneClass($item['tone'], 'light') }}">
                            <i class="fa-solid {{ $item['icon'] }}"></i>
                        </div>
                        <div>
                            <p class="text-sm font-bold {{ $active ? 'text-white' : 'text-slate-700' }}">{{ $item['label'] }}</p>
                        </div>
                    </a>
                @endforeach
            </div>
        ...
        <div class="widget-card p-6 flex flex-col justify-center">
            <p class="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-4">Pilih Tanggal</p>
            <form method="GET" action="{{ route('kader.absensi.index') }}">
                <input type="hidden" name="kategori" value="{{ $kategoriAktif }}">
                <div class="flex gap-2">
                    <input type="date" name="tanggal" value="{{ $tanggalInput }}" max="{{ now('Asia/Jakarta')->toDateString() }}" class="w-full btn-pill border border-slate-200 bg-white/50 px-4 py-3 text-sm font-bold text-slate-700 outline-none transition focus:border-teal-400 focus:bg-white focus:ring-4 focus:ring-teal-500/10 shadow-inner">
                    <button type="submit" class="btn-pill bg-teal-500 px-5 text-white shadow-[0_5px_15px_rgba(20,184,166,0.3)] hover:bg-teal-600 hover:-translate-y-0.5">
    ...
    {{-- 4. MAIN FORM GRID --}}
    <form id="attendanceForm" method="POST" action="{{ route('kader.absensi.store') }}" class="flex flex-col xl:flex-row gap-6 items-stretch">
        @csrf
        <input type="hidden" name="kategori" value="{{ $kategoriAktif }}">
        <input type="hidden" name="tanggal" value="{{ $tanggalInput }}">

        {{-- LEFT: ATTENDANCE LIST --}}
        <section class="widget-card flex-1 flex flex-col p-6 xl:h-[780px]">
            <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-5 shrink-0 border-b border-slate-100 pb-5">
                <div>
                    <h2 class="text-lg font-black text-slate-800">Daftar Presensi</h2>
                    <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mt-1" id="visibleCountText">Menampilkan {{ number_format($totalSasaran) }} data.</p>
                </div>
                
                <div class="flex flex-wrap gap-2">
                    <button type="button" id="markAllPresent" class="btn-pill bg-emerald-50 hover:bg-emerald-100 text-emerald-600 border border-emerald-200 px-4 py-2 text-[11px] font-bold uppercase tracking-wider transition-colors shadow-sm">Hadir Semua</button>
                    <button type="button" id="markAllAbsent" class="btn-pill bg-amber-50 hover:bg-amber-100 text-amber-600 border border-amber-200 px-4 py-2 text-[11px] font-bold uppercase tracking-wider transition-colors shadow-sm">Absen Semua</button>
                    <button type="button" id="resetChanges" class="btn-pill border border-slate-200 bg-white/50 text-slate-500 px-3 py-2 text-[11px] font-bold hover:bg-slate-50 shadow-sm" title="Reset Semua"><i class="fa-solid fa-rotate-left"></i></button>
                </div>
            </div>

            {{-- SEARCH BAR DIPERBAIKI (TOMBOL X BERFUNGSI) --}}
            <div class="relative mb-5 shrink-0 group">
                <i class="fa-solid fa-magnifying-glass absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 z-10 pointer-events-none"></i>
                <input type="text" id="attendanceSearch" autocomplete="off" placeholder="Cari nama atau NIK sasaran..." class="w-full btn-pill border border-slate-200 bg-white/50 py-3.5 pl-11 pr-12 text-sm font-semibold text-slate-800 outline-none transition focus:bg-white focus:border-teal-400 focus:ring-4 focus:ring-teal-500/10 shadow-inner relative z-0">
                <button type="button" id="clearSearch" class="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-full transition-all hidden z-20 cursor-pointer" title="Bersihkan Pencarian">
                    <i class="fa-solid fa-xmark text-lg pointer-events-none"></i>
                </button>
            </div>

            @if($pasiens->isNotEmpty())
                <div id="attendanceList" class="flex-1 slim-scroll overflow-y-auto pr-2 space-y-3 pb-4">
                    @foreach($pasiens as $index => $pasien)
                        ...
                        <div class="pc-row p-4 {{ $rowState }}"
                            ...
                            <div class="flex flex-col xl:flex-row justify-between xl:items-center gap-4">
                                ...
                                {{-- IDENTITAS DIPERBAIKI (TIDAK TUMPANG TINDIH & VARIABEL BENAR) --}}
                                <div class="flex items-start sm:items-center gap-4 min-w-0 flex-1">
                                    <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-400 shadow-sm">
                                        <i class="fa-solid fa-user text-lg"></i>
                                    </div>
                                    <div class="min-w-0 flex-1">
                                        <h3 class="text-base sm:text-lg font-black text-slate-800 break-words leading-tight mb-1" title="{{ $nama }}">
                                            {{ $nama }}
                                        </h3>
                                        <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] font-semibold text-slate-500">
                                ...
                                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full xl:w-auto shrink-0 pl-16 xl:pl-0">
                                    <div class="flex gap-2 w-full sm:w-auto bg-slate-100/50 p-1 rounded-[1.5rem] border border-slate-100">
                                        <button type="button" data-choice="1" class="pc-choice-btn btn-pill flex-1 sm:flex-none border border-transparent bg-white px-6 py-2.5 text-xs font-bold text-slate-500 hover:bg-slate-50 hover:text-slate-700 shadow-sm {{ $stateValue === '1' ? 'btn-present-active' : '' }}">
                                            <i class="fa-solid fa-check mr-1 opacity-50"></i> Hadir
                                        </button>
                                        <button type="button" data-choice="0" class="pc-choice-btn btn-pill flex-1 sm:flex-none border border-transparent bg-white px-6 py-2.5 text-xs font-bold text-slate-500 hover:bg-slate-50 hover:text-slate-700 shadow-sm {{ $stateValue === '0' ? 'btn-absent-active' : '' }}">
                                            <i class="fa-solid fa-xmark mr-1 opacity-50"></i> Tidak
                                        </button>
                                        <button type="button" data-reset-row class="btn-pill w-10 flex justify-center items-center border border-transparent bg-white text-slate-400 hover:bg-slate-50 hover:text-slate-600 shadow-sm" title="Reset">
                                            <i class="fa-solid fa-rotate-left text-[11px]"></i>
                                        </button>
                                    </div>
                                    <input type="text" name="keterangan[{{ $pasien->id }}]" value="{{ $keterangan }}" data-note-input placeholder="Catatan opsional..." class="w-full sm:w-48 btn-pill border border-slate-200 bg-white/70 px-4 py-2.5 text-xs font-medium text-slate-700 outline-none transition focus:border-teal-400 focus:bg-white focus:ring-2 focus:ring-teal-500/10 mt-2 sm:mt-0 shadow-inner">
                                </div>
                            </div>
                        </div>
                    @endforeach
            ...
            @else
                <div class="flex-1 flex flex-col items-center justify-center btn-pill border-2 border-dashed border-slate-200 bg-white/50 p-8 text-center rounded-[2rem]">
                    <div class="flex h-16 w-16 items-center justify-center rounded-full bg-slate-100 text-slate-400 mb-4 shadow-inner">
                        <i class="fa-solid fa-users-slash text-2xl"></i>
                    </div>
                    <h3 class="font-bold text-slate-800 text-lg">Data Kosong</h3>
                    <p class="text-sm text-slate-500 mt-1 max-w-sm">Tambahkan sasaran terlebih dahulu pada menu Data Pasien di Posyandu Management System.</p>
                </div>
            @endif
        </section>

        {{-- RIGHT: SUMMARY INPUT --}}
        <aside class="widget-card w-full xl:w-[360px] flex flex-col p-6 xl:h-[780px] shrink-0 bg-gradient-to-b from-white/90 to-white/60">
            <div class="flex-1 flex flex-col">
                <div class="mb-6 border-b border-slate-100 pb-5">
                    <p class="text-[11px] font-black uppercase tracking-wider text-teal-500 mb-1 flex items-center gap-2">
                        <i class="fa-solid fa-shield-halved"></i> Validasi Form
                    </p>
                    <h2 class="text-lg font-black text-slate-800">Summary Input</h2>
                    <p class="mt-2 text-xs font-medium text-slate-500 leading-relaxed">Pastikan semua status kehadiran telah menyala/terpilih sebelum klik simpan data presensi.</p>
                </div>

                <div class="space-y-3">
                    <div class="btn-pill border border-slate-100 bg-white/80 p-4 flex justify-between items-center shadow-sm">
                        <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Kategori</span>
                        <span class="text-sm font-black text-slate-800">{{ $currentKategori['label'] }}</span>
                    </div>
                    <div class="btn-pill border border-slate-100 bg-white/80 p-4 flex justify-between items-center shadow-sm">
                        <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Tanggal</span>
                        <span class="text-sm font-black text-slate-800">{{ $tanggalPendek }}</span>
                    </div>
                    <div class="btn-pill border border-slate-100 bg-white/80 p-4 flex justify-between items-center shadow-sm">
                        <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Status</span>
                        <span class="text-[9px] font-black uppercase tracking-wider btn-pill px-3 py-1.5 shadow-inner {{ $sesiHariIni ? 'bg-sky-100 text-sky-600' : 'bg-teal-100 text-teal-600' }}">
                            <i class="fa-solid {{ $sesiHariIni ? 'fa-pen-to-square' : 'fa-plus' }} mr-1"></i> {{ $sesiHariIni ? 'Update' : 'Baru' }}
                        </span>
                    </div>
                </div>

                <div class="mt-6 rounded-[2rem] border border-slate-100 bg-white p-6 shadow-lg shadow-slate-100/50 flex-1 flex flex-col justify-center relative overflow-hidden">
                    <div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-teal-400 to-emerald-400"></div>
                    <p class="text-[11px] font-black uppercase tracking-wider text-slate-400 mb-4 text-center">Progress Sesi</p>
                    
                    <div class="flex justify-center items-end gap-2 mb-4">
                        <span class="text-5xl font-black text-teal-500 drop-shadow-sm" id="sideHadirText">{{ $totalHadir }}</span>
                        <span class="text-base font-bold text-slate-400 mb-1.5">/ {{ $totalSasaran }}</span>
                    </div>

                    <div class="h-3 w-full btn-pill bg-slate-100 overflow-hidden shadow-inner">
                    ...
                    <div class="text-center mt-3">
                        <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider" id="sideProgressText">{{ $persenHadir }}% Sasaran Hadir</span>
                    </div>
                </div>
            </div>
            ...
            @if($pasiens->isNotEmpty())
                <div class="mt-6 pt-2">
                    <button type="submit" id="submitAttendanceBtn" class="w-full flex items-center justify-center gap-2 btn-pill bg-gradient-to-r from-teal-500 to-emerald-500 hover:from-teal-600 hover:to-emerald-600 px-5 py-4 text-sm font-bold text-white shadow-[0_10px_25px_rgba(20,184,166,0.4)] hover:-translate-y-1 transition-all">
                        <i class="fa-solid fa-cloud-arrow-up text-lg"></i> Simpan Presensi
                    </button>
                </div>
            @endif
        </aside>
    ...
    <div id="nexusAlertModal" class="nexus-modal-overlay fixed inset-0 z-[999999] hidden items-center justify-center bg-slate-900/40 backdrop-blur-md opacity-0 p-4">
        <div class="nexus-modal-content relative w-full max-w-md transform scale-90 rounded-[2.5rem] bg-white/95 p-10 border border-white/80 overflow-hidden shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)]">
            ...
            <div class="relative z-10 flex flex-col items-center">
                <div class="mx-auto flex h-24 w-24 items-center justify-center relative mb-6">
                    <div class="absolute inset-0 bg-amber-400 rounded-full blur-xl opacity-40 orb-glow"></div>
                    <div class="relative flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-amber-100 to-amber-50 border border-amber-200 text-amber-500 shadow-inner">
                        <i class="fa-solid fa-triangle-exclamation text-4xl drop-shadow-md"></i>
                    </div>
                </div>
                
                <h3 class="text-center text-2xl font-black text-slate-800 tracking-tight">Perhatian!</h3>
                <div class="w-12 h-1.5 bg-amber-400 rounded-full mx-auto mt-3 mb-5"></div>
                
                <p id="nexusAlertMessage" class="text-center text-base font-medium text-slate-600 leading-relaxed px-2 max-w-sm">
                ...
                <div class="mt-8 w-full">
                    <button type="button" id="nexusAlertClose" class="w-full flex items-center justify-center btn-pill bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 px-6 py-4 text-base font-bold text-white shadow-[0_10px_25px_rgba(245,158,11,0.4)] hover:-translate-y-1 transition-all">
                        <i class="fa-solid fa-check-circle text-xl mr-2"></i> Saya Mengerti
                    </button>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // --- FUNGSI SEARCH DAN TOMBOL X DIPERBAIKI ---
