...
<style>
    ...
    /* Widget Card Super Membulat (Squircle konsisten) */
    .widget-card {
    ...
    /* Fix Tooltip ApexChart agar membulat penuh */
    .apexcharts-tooltip {
...
<div class="px-3 py-4 sm:px-6 sm:py-8 lg:px-8 max-w-[1400px] mx-auto space-y-4 sm:space-y-8 overflow-x-hidden">
    ...
    <div class="relative overflow-hidden rounded-[2.5rem] sm:rounded-[3rem] bg-gradient-to-r from-teal-500 via-teal-400 to-emerald-400 p-5 sm:p-10 shadow-2xl shadow-teal-500/20 flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-5 sm:gap-8 border-[4px] sm:border-[6px] border-white/40">
        ...
        <div class="relative z-10 w-full lg:w-1/2 flex flex-col gap-3 sm:gap-5 text-center lg:text-left">
            <div class="inline-flex justify-center lg:justify-start items-center gap-2">
                <span class="btn-pill bg-white/20 border border-white/30 text-white px-4 py-1.5 text-[10px] sm:text-xs font-black uppercase tracking-widest backdrop-blur-md shadow-inner flex items-center gap-2">
            ...
            <p class="text-teal-50 text-xs sm:text-sm font-medium leading-relaxed max-w-md mx-auto lg:mx-0">
                Pusat operasional Posyandu terpadu[cite: 13]. Kelola data sasaran, pantau tren kehadiran, dan rekam hasil pengukuran secara real-time[cite: 13].
            </p>
        ...
        <div class="relative z-10 w-full lg:w-auto grid grid-cols-2 sm:flex sm:flex-row gap-3 sm:gap-4 justify-center items-stretch">
            ...
            {{-- Jam Realtime Panel --}}
            <div class="widget-card !rounded-[2rem] sm:!rounded-[2.5rem] !bg-white/80 p-4 sm:p-6 flex flex-col items-center justify-center sm:min-w-[200px]">
                <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-teal-100 text-teal-500 flex items-center justify-center text-sm sm:text-xl mb-2 sm:mb-3 shadow-inner">
            ...
            {{-- Ringkasan Bulanan --}}
            <div class="widget-card !rounded-[2rem] sm:!rounded-[2.5rem] !bg-white/80 p-4 sm:p-6 flex flex-col justify-center gap-3 sm:gap-4 sm:min-w-[200px]">
                <div class="flex items-center gap-3 sm:gap-4">
                    <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-emerald-100 text-emerald-500 flex items-center justify-center text-sm sm:text-xl shrink-0 shadow-inner">
                ...
                <div class="flex items-center gap-3 sm:gap-4">
                    <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-blue-100 text-blue-500 flex items-center justify-center text-sm sm:text-xl shrink-0 shadow-inner">
    ...
    {{-- 2. METRIK UTAMA (4 WIDGETS) --}}
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
        @foreach($cardStats as $card)
            <a href="{{ $card['url'] }}" class="widget-card p-4 sm:p-6 group block flex flex-col justify-between min-w-0">
                <div class="flex justify-between items-center mb-3 sm:mb-5">
                    <div class="w-11 h-11 sm:w-14 sm:h-14 rounded-full bg-{{ $card['theme'] }}-50 text-{{ $card['theme'] }}-500 flex items-center justify-center text-base sm:text-2xl shrink-0 group-hover:bg-{{ $card['theme'] }}-500 group-hover:text-white group-hover:shadow-lg group-hover:shadow-{{ $card['theme'] }}-500/30 transition-all duration-300 shadow-inner">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6 items-stretch">
        ...
        <div class="xl:col-span-2 flex flex-col gap-4 sm:gap-6">
            ...
            <div class="grid grid-cols-3 gap-2 sm:gap-4">
                @foreach($breakdowns as $b)
                    <a href="{{ $b['url'] }}" class="widget-card !rounded-[2rem] p-3 sm:p-4 sm:pr-6 flex flex-col sm:flex-row items-center gap-2 sm:gap-4 text-center sm:text-left hover:border-{{ $b['color'] }}-300 min-w-0">
                        <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-{{ $b['color'] }}-100 text-{{ $b['color'] }}-600 flex items-center justify-center text-sm sm:text-xl shrink-0 shadow-inner">
            ...
            <div class="widget-card p-4 sm:p-6 lg:p-8 flex-1 flex flex-col min-h-[300px] sm:min-h-[350px]">
                <div class="flex flex-col sm:flex-row justify-between items-stretch sm:items-center mb-4 sm:mb-6 gap-3 sm:gap-4 bg-slate-50/50 p-3 sm:p-2 sm:pl-6 rounded-[2rem] border border-slate-100">
                    <div class="text-center sm:text-left">
                        <h2 class="text-xs sm:text-sm font-bold text-slate-800 uppercase tracking-widest">Tren Aktivitas</h2>
                    </div>
                    <div class="flex w-full sm:w-auto shrink-0 bg-white p-1 rounded-full shadow-inner border border-slate-100 relative">
                        @foreach([7, 14, 30] as $days)
                            <button type="button" data-range="{{ $days }}" class="trend-range btn-pill flex-1 sm:flex-initial px-3 sm:px-4 py-1.5 sm:py-2 text-[11px] sm:text-xs font-bold transition-all duration-300 {{ $days === 7 ? 'bg-teal-500 text-white shadow-md' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-50' }}">
                                {{ $days }} Hari
                            </button>
                        @endforeach
                    </div>
        ...
        <div class="widget-card flex flex-col p-3">
            <div class="p-3 sm:p-4 flex items-center justify-between pb-3 border-b border-slate-100 gap-2">
                <div class="flex items-center gap-3 min-w-0">
                    <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-amber-100 text-amber-500 flex items-center justify-center text-sm sm:text-lg shadow-inner shrink-0">
                ...
                @if(Route::has('kader.jadwal.index'))
                    <a href="{{ route('kader.jadwal.index') }}" class="btn-pill w-9 h-9 sm:w-10 sm:h-10 bg-slate-50 hover:bg-teal-50 border border-slate-100 text-slate-400 hover:text-teal-600 flex items-center justify-center transition-all shrink-0 text-xs sm:text-base shadow-sm">
                        <i class="fa-solid fa-arrow-right"></i>
                    </a>
                @endif
            </div>

            <div class="flex-1 overflow-y-auto slim-scroll p-2 sm:p-3 space-y-3 max-h-[320px] xl:max-h-[100%]">
                @forelse($jadwalMendatang as $jadwal)
                    ...
                    <a href="{{ Route::has('kader.jadwal.show') ? route('kader.jadwal.show', $jadwal->id) : '#' }}" 
                       class="group block rounded-[2rem] p-3.5 border transition-all {{ $isToday ? 'bg-gradient-to-r from-amber-50 to-orange-50 border-amber-200 shadow-sm' : 'bg-white/80 border-slate-100 hover:border-teal-200 hover:shadow-md' }}">
                        <div class="flex gap-3.5 items-center">
                            <div class="flex flex-col items-center justify-center w-12 h-12 sm:w-14 sm:h-14 rounded-full shrink-0 shadow-inner {{ $isToday ? 'bg-amber-500 text-white shadow-amber-500/30' : 'bg-slate-100 text-slate-600 group-hover:bg-teal-500 group-hover:text-white' }} transition-colors">
                                <span class="text-base sm:text-lg font-black leading-none">{{ $formatDate($jadwal->tanggal ?? null, 'd') }}</span>
                                <span class="text-[9px] font-bold uppercase tracking-widest mt-0.5">{{ $formatDate($jadwal->tanggal ?? null, 'M') }}</span>
                            </div>
                            <div class="min-w-0 flex-1 pr-1">
                                <div class="flex items-center gap-2 mb-1">
                                    <h3 class="truncate text-xs sm:text-sm font-bold text-slate-800">{{ $jadwal->judul ?? 'Agenda' }}</h3>
                                    @if($isToday)
                                        <span class="btn-pill bg-amber-500 px-2 py-0.5 text-[9px] font-black text-white uppercase tracking-wider shadow-sm">HARI INI</span>
                                    @endif
                                </div>
                                <p class="btn-pill inline-flex bg-slate-50 border border-slate-100 px-2.5 py-1 text-[10px] font-bold text-slate-500 items-center gap-1.5 shadow-inner">
                ...
                @empty
                    <div class="flex h-full flex-col items-center justify-center text-center opacity-70 p-6 sm:p-8 min-h-[200px] sm:min-h-[250px]">
                        <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-[2rem] bg-slate-100 flex items-center justify-center mb-3 sm:mb-4 border border-slate-200 shadow-inner">
    ...
    document.querySelectorAll('.trend-range').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            document.querySelectorAll('.trend-range').forEach(item => {
                item.className = 'trend-range btn-pill flex-1 sm:flex-initial px-3 sm:px-4 py-1.5 sm:py-2 text-[11px] sm:text-xs font-bold transition-all duration-300 text-slate-500 hover:text-slate-800 hover:bg-slate-50';
            });
            this.className = 'trend-range btn-pill flex-1 sm:flex-initial px-3 sm:px-4 py-1.5 sm:py-2 text-[11px] sm:text-xs font-bold transition-all duration-300 bg-teal-500 text-white shadow-md';
