...
@php
    ...
    $toneMap = [
        'emerald' => [
            'icon_bg' => 'bg-emerald-50', 'icon_text' => 'text-emerald-500',
            'badge' => 'bg-emerald-50 text-emerald-600 border-emerald-200',
            'button' => 'bg-white text-emerald-600 border-emerald-200 hover:bg-emerald-500 hover:text-white',
        ],
        'rose' => [
            'icon_bg' => 'bg-rose-50', 'icon_text' => 'text-rose-500',
            'badge' => 'bg-rose-50 text-rose-600 border-rose-200',
            'button' => 'bg-white text-rose-600 border-rose-200 hover:bg-rose-500 hover:text-white',
        ],
        'sky' => [
            'icon_bg' => 'bg-sky-50', 'icon_text' => 'text-sky-500',
            'badge' => 'bg-sky-50 text-sky-600 border-sky-200',
            'button' => 'bg-white text-sky-600 border-sky-200 hover:bg-sky-500 hover:text-white',
        ],
        'amber' => [
            'icon_bg' => 'bg-amber-50', 'icon_text' => 'text-amber-500',
            'badge' => 'bg-amber-50 text-amber-600 border-amber-200',
            'button' => 'bg-white text-amber-600 border-amber-200 hover:bg-amber-500 hover:text-white',
        ],
        'violet' => [
            'icon_bg' => 'bg-violet-50', 'icon_text' => 'text-violet-500',
            'badge' => 'bg-violet-50 text-violet-600 border-violet-200',
            'button' => 'bg-white text-violet-600 border-violet-200 hover:bg-violet-500 hover:text-white',
        ],
        'slate' => [
            'icon_bg' => 'bg-slate-50', 'icon_text' => 'text-slate-500',
            'badge' => 'bg-slate-50 text-slate-600 border-slate-200',
            'button' => 'bg-white text-slate-600 border-slate-200 hover:bg-slate-500 hover:text-white',
        ],
    ];
@endphp
...
<style>
    ...
    /* Transisi smooth saat konten berubah */
    #notifSection .flex-col {
        transition: opacity 0.15s ease;
    }
    #notifSection .flex-col.fade {
        opacity: 0.5;
    }
</style>
@endpush
...
<div class="max-w-[1280px] mx-auto animate-pop-in pb-20 px-4 sm:px-6 lg:px-8 mt-6 space-y-6">
    ...
    <section class="bg-gradient-to-br from-emerald-500 via-teal-500 to-teal-600 rounded-[2.5rem] p-8 md:p-10 relative overflow-hidden shadow-[0_20px_40px_-12px_rgba(20,184,166,.35)] border border-white/20 text-center md:text-left flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div class="relative z-10 flex-1">
            ...
            <p class="text-white/80 text-sm font-medium mt-3 leading-relaxed max-w-xl mx-auto md:mx-0">
                Pusat informasi jadwal, hasil pemeriksaan, imunisasi, dan pemberitahuan penting Posyandu untuk keluarga Anda.
            </p>
        </div>
    ...
    {{-- 2. TABS & INSTANT AJAX SEARCH FORM --}}
    <div class="bg-white/70 backdrop-blur-xl border border-slate-200 shadow-sm rounded-[2rem] p-5 flex flex-col xl:flex-row gap-5 justify-between">
        ...
        <div class="flex flex-wrap gap-2 items-center">
            @foreach($tabs as $tab)
                ...
                @endphp
                <a href="{{ $tabUrl }}" class="btn-pill h-11 px-4 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest border transition-all {{ $isActive ? 'bg-emerald-500 border-emerald-500 text-white shadow-md' : 'bg-slate-50 border-slate-200 text-slate-500 hover:bg-slate-100 hover:text-slate-700' }}">
        ...
        <form action="{{ $indexRoute }}" method="GET" id="searchForm" class="flex-1 flex gap-2 xl:justify-end">
            ...
            <div class="relative w-full xl:w-72">
                <input type="text" id="liveSearch" name="search" value="{{ $filters['search'] ?? '' }}" autocomplete="off" placeholder="Ketik untuk mencari instan..." class="input-soft w-full pl-10 pr-10 h-11">
    ...
    {{-- 3. CONTAINER MASTER --}}
    <section id="notifSection" class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden mb-8">
        ...
        <div class="bg-slate-50/70 px-6 sm:px-8 py-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <h5 class="font-black text-slate-700 text-sm uppercase tracking-widest flex items-center gap-2">
                ...
                @if(filled($filters['search']))
                    Hasil Pencarian: "<span id="searchQueryDisplay">{{ $filters['search'] }}</span>"
                @else
                    Daftar Pesan
                @endif
            </h5>
        ...
        {{-- Daftar Pesan --}}
        <div class="flex flex-col divide-y divide-slate-100" id="notifList">
                ...
                <div class="flex flex-col lg:flex-row gap-5 p-6 sm:px-8 transition-colors duration-200 {{ !$card['is_read'] ? 'bg-emerald-50/30' : 'hover:bg-slate-50/50' }}">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex flex-wrap items-center gap-2 mb-2">
                            <span class="inline-block px-2.5 py-0.5 rounded text-[8px] font-black uppercase tracking-widest border {{ $tone['badge'] }}">
                                {{ $card['label'] }}
                            </span>
                            @if(!$card['is_read'])
                                <span class="inline-block px-2.5 py-0.5 rounded text-[8px] font-black uppercase tracking-widest border border-rose-200 bg-rose-50 text-rose-700">
                                    <i class="fas fa-circle text-[5px] mr-0.5 align-middle"></i> Baru
                                </span>
                            @endif
                            <span class="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-auto lg:ml-2">
                                <i class="fas fa-clock mr-0.5"></i> {{ $card['waktu'] }}
                            </span>
                        </div>

                        <h3 class="text-base font-black text-slate-800 leading-tight mb-1" title="{{ $card['judul'] }}">
                            {{ $card['judul'] }}
                        </h3>
                        <p class="text-xs font-semibold text-slate-600 leading-relaxed max-w-4xl">
                            {{ $card['pesan'] }}
                        </p>
                    ...
                    <div class="w-full lg:w-40 flex flex-col justify-center gap-2 shrink-0 border-t lg:border-t-0 lg:border-l border-slate-100 pt-4 lg:pt-0 lg:pl-6 mt-2 lg:mt-0">
                        ...
                        @if(($card['link'] ?? '#') !== '#')
                            <a href="{{ $card['link'] }}" class="btn-pill w-full text-center px-4 py-2.5 rounded-[12px] text-[10px] font-black uppercase tracking-widest border shadow-sm transition-all {{ $tone['button'] }}">
            ...
            @empty
                <div class="p-16 text-center bg-slate-50/50" id="emptyState">
                    <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-[1.5rem] bg-white border border-slate-200 text-2xl text-slate-300 shadow-sm mb-4">
                        <i class="fas fa-search"></i>
                    </div>
                    ...
                    @if(filled($filters['search']))
                        ...
                        <p class="mx-auto mt-1 max-w-sm text-sm font-semibold leading-6 text-slate-500">
                            Tidak ada pesan yang cocok dengan <strong class="text-slate-700">"{{ $filters['search'] }}"</strong>.
                        </p>
                    @else
                        <h3 class="text-lg font-black text-slate-800">Kotak Masuk Bersih</h3>
                        <p class="mx-auto mt-1 max-w-sm text-sm font-semibold leading-6 text-slate-500">
                            Tidak ada pesan atau pemberitahuan pada kategori ini.
                        </p>
                    @endif
                </div>
            @endforelse
        ...
        {{-- CUSTOM PAGINATION --}}
        @if(method_exists($notifikasis ?? collect(), 'links') && isset($notifikasis) && $notifikasis->hasPages())
            <div class="border-t border-slate-100 bg-slate-50/80 px-6 sm:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-1.5">
                    ...
                    @else
                        <a href="{{ $notifikasis->previousPageUrl() }}" class="btn-pill w-8 h-8 flex items-center justify-center border border-slate-200 bg-white text-slate-600 shadow-sm hover:bg-slate-100 transition-all"><i class="fas fa-chevron-left text-[10px]"></i></a>
                    @endif
                    ...
                    @for ($page = $start; $page <= $end; $page++)
                        ...
                        @else
                            <a href="{{ $notifikasis->url($page) }}" class="btn-pill w-8 h-8 flex items-center justify-center border border-slate-200 bg-white text-slate-600 font-bold text-[10px] shadow-sm hover:bg-slate-100 transition-all">{{ $page }}</a>
                        @endif
                    @endfor

                    @if ($notifikasis->hasMorePages())
                        <a href="{{ $notifikasis->nextPageUrl() }}" class="btn-pill w-8 h-8 flex items-center justify-center border border-slate-200 bg-white text-slate-600 shadow-sm hover:bg-slate-100 transition-all"><i class="fas fa-chevron-right text-[10px]"></i></a>
                    @else
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (!searchInput || !searchForm || !notifSection) return;

        let debounceTimer;
        let fetchController;

        // Cegah halaman me-reload penuh jika user menekan Enter
        searchForm.addEventListener('submit', function(e) {
            e.preventDefault();
        });

        // Simpan nilai awal pencarian untuk mengecek apakah berubah
        ...
        searchInput.addEventListener('input', function () {
            ...
            if (newValue === currentSearchValue && newValue !== '') {
                return;
            }
            ...
            // Debounce sangat cepat: 80ms (agar terasa instan)
            debounceTimer = setTimeout(() => {
                ...
                for (const [key, value] of formData.entries()) {
                    if (key === 'search') {
                        // Gunakan nilai terbaru dari input
                ...
                .then(html => {
                    // Ekstrak bagian daftar pesan dari respons HTML
                    const parser = new DOMParser();
                    const doc = parser.parseFromString(html, 'text/html');
                    const newSection = doc.getElementById('notifSection');
                    
                    // Timpa daftar lama secara instan
                    if (newSection) {
                        const newNotifList = newSection.querySelector('#notifList');
                        if (newNotifList) {
                            notifList.innerHTML = newNotifList.innerHTML;
                            // Update header judul jika ada kata kunci
                            const headerTitle = notifSection.querySelector('.bg-slate-50\\/70 h5');
                            if (headerTitle && newValue) {
                                const newHeader = newSection.querySelector('.bg-slate-50\\/70 h5');
                                if (newHeader) {
                                    headerTitle.innerHTML = newHeader.innerHTML;
                                }
                            }
                        }
                        // Update empty state juga jika ada
                        const emptyState = newSection.querySelector('#emptyState');
                        if (emptyState) {
                            const currentEmpty = notifSection.querySelector('#emptyState');
                            if (currentEmpty) {
                                currentEmpty.innerHTML = emptyState.innerHTML;
                            }
                        }
                    }

                    // Sinkronkan URL di browser agar search tetap di URL
                    const newUrl = new URL(url);
                    window.history.pushState({}, '', newUrl);
                })
                .catch(error => {
                    // Abaikan jika error karena aborted
                    if (error.name !== 'AbortError') {
                        console.warn('Pencarian instan gagal:', error);
                    }
                })
                .finally(() => {
        ...
        // Saat user menghapus semua teks, kembalikan ke daftar semua
        searchInput.addEventListener('keydown', function(e) {
            // Jika tombol Backspace dan nilai kosong, langsung trigger
            if (e.key === 'Backspace' && this.value.length === 1) {
                // Hapus teks secara virtual agar 'input' event terpicu
                setTimeout(() => {
        ...
        // Tambahkan event untuk pencarian ulang saat tab filter berubah
        document.querySelectorAll('.btn-pill[href*="filter"]').forEach(link => {
            link.addEventListener('click', function(e) {
                ...
                // Trigger search ulang dengan kata kunci yang sama
                const searchValue = searchInput.value;
                if (searchValue) {
                    // Kirim ulang pencarian dengan filter baru
                    searchInput.dispatchEvent(new Event('input'));
                } else {
                    // Tanpa pencarian, reload halaman dengan filter baru
